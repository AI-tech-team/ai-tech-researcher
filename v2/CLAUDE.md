...
6. **フロントは信用しない（アーキテクチャ原則・2026-07-07制定）**: クライアント由来の値（id/フォーム入力/クエリ/ヘッダ/Cookie以外のセッション主張）は**すべてバックエンドで検証**する。認可はサーバ側セッション（`isOwner()`/`currentUserId()`）でスコープし、入力はzod＋長さ上限で検証、クライアント側バリデーションはUX補助にすぎない。新規のServer Action/APIルートはこの原則を満たさない限り追加しない。

7. **主張の前に測る（prove-it・2026-07-16制定）**: 原因・影響・閾値を**印象で断定しない**。「〜のはず」「これは全部○○だ」と書きかけたら止まり、実データで反証可能な形にして測る（分布・規模別・反証例・実物のサンプル）。指標で分離できないと分かったら無理に線を引かず、**失敗の非対称性**（サイレントな消失＞＞見える冗長）で選ぶ。誤った自分の主張は明示的に撤回する。手順は [[prove-it]]。

着手前は [[vibe-check]] の6条、非自明な断定・診断・閾値決めの前は [[prove-it]]、risky な変更のコミット前は [[precheck]]（マルチエージェント・レビュー）を回す。
