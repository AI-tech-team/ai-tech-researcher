...
## 2026-09-13 ⑪ 同じモデルが3回並んでいた（量子化再アップ）

`/articles` の第一画面を見に行って見つけた3件目。今日追加した `[モデル公開]` の8件中、3件が同じモデルだった:

```
Qwen/Qwen3.8-27B                      ★9
ISTA-DASLab/Qwen3.8-27B-GSQ-RCO-GGUF  ★7   ← 量子化再アップ
unsloth/Qwen3.8-27B-GGUF              ★6   ← 同上
```

HF trending 30件のうち**5件(17%)** がこの形（GGUF / FP8 等）。量子化して再アップされたものは
**別の出来事ではない**。同じ公開が3行に増えるのは、朝刊でも一覧でもノイズにしかならない。

**決定: 「本体が既にあるなら重ねない」。「派生だから捨てる」ではない。**
収集ゲートで落としたものは二度と戻らない（[[pattern-filter-by-recoverability]]）ので、
派生しかトレンドに乗らない日はそれを拾う。判定は `src/lib/model-id.ts`（テスト9件）。

- **著者名では判定しない。**`ISTA-DASLab` も `unsloth` も独自モデルを出すし、
  本物の研究組織も量子化版を再配布する。判定は名前の印（`-GGUF` `-AWQ` `-FP8` 等）だけ。
- バッチ内の畳み込みに加え、**直近7日の `[モデル公開]` と照合**する。
  本体を昨日拾って今日その量子化版が伸びる場合、バッチ内だけでは防げない。
- 畳むのは `slice(0,10)` の**前**。逆にすると上限10件の枠を重複が食う（[[pattern-throughput-starvation]]）。

### 直さなかったもの: GitHub trending の中身

同じ画面に `[GitHub] shengjidaguai-china/goutoujunshi`（恋愛軍師）や
`Tiger3807861189/J-Space-Cognition-Suite-V3.7`（個人のスキル集）が並んでいて気になったが、
**測ったら直す理由が無かった**。10件とも star 200超の実在のトレンドで、
importance は 5〜8 に散っており朝刊の★9には届かない＝一覧（見逃しチェックの面）に出ているだけ。
**私の違和感は美観の話で、不具合ではなかった。**

### /articles について測って分かったこと（当初の仮説は外れ）

「★9の個人投稿が一覧の先頭を占めているのでは」と考えて測ったが、外れていた。

- 並びは**重要度順ではなく `created_at DESC` の純粋な新着順**
- 先頭20件の個人投稿は **0%**、先頭60件でも **8%**（31%ではない）
- 参考に重要度順を測ると、★10の先頭10件のうち**4件が既に停止したノイズ源**
  （pasqualepillitteri / startupfortune×2 / digitimes）＝**新着順のままが正しい**

並べ替えは**しない**。

---

## 2026-09-13 ⑫ 収集の「絞る」が全部あべこべだった（4本／位置に意味を仮定していた）

⑩で「LLMの返り値を位置で突き合わせるな」を直した直後に、**入り口側でも同じことをしていた**と分かった。
今度は相手がLLMではなく**外部API・RSSフィード・HNの順位**で、こちらが勝手に
「返ってくる順＝重要度順／新しい順」と読んでいた。全部、取得だけで実測できた（DB書込もLLM課金も無し）。

### 測ったもの（2026-09-13・本番と同じクエリ）

| 収集 | 実測 | 何が起きていたか |
|---|---|---|
| HF papers | 30件中、上位8件が**8件とも先頭8件の外** | 👍444の筆頭論文（27番目）を捨て👍20・👍8を評価。票合計 **200 対 1209** |
| HN | top150に条件合致**13件**、break は順位21で停止 | ▲1191 / ▲932 / ▲666 / OpenAI Agents API を**一度も見ていない**。拾うのは▲129・▲131 |
| RSS | 40本中 読めた34本の**6本(17.6%)が降順でない** | NVIDIA Developer Blog は最新が2番目。先頭から切ると新しい記事を落とす |
| arXiv | `max_results=30` が**全部同じ投稿日** | 1日分にも足りない窓から先頭10件。残り20件は二度と来ない |

### 決定: 収集の順序を1つに統一する

**広く取る → 重複を除く → 意味のある順に並べ替える → 上限で絞る。**

現場で壊れていたのは例外なく「絞る」が先に来ている形だった。上位が既知になった日に静かに0件になり、
`last_hit_at` は更新されるので**どこにも異常が出ない**（github-trending の77日と同じ）。
並べ替えは `topByScore()`（`src/lib/collect-select.ts`・テスト6件）に一本化した。
票・スコア・日付を同じ規則で扱い、同点は元の並びを保つ（安定ソート）。

- **HN は打ち切りをやめて150件を最後まで見る**。10並列で**実測3.8秒**（逐次より速い）。
  評価上限は 5 → **10**。1ソース1バッチ＝**LLM呼び出しは1回のまま**で、増えるのは
  1回あたりのトークンだけ（gemini-2.5-flash-lite）。実測13件に対し10なら取りこぼしは3件。
- 上限は名前付き定数にした（`RSS_EVAL_MAX` / `ARXIV_EVAL_MAX` / `HN_SCAN_DEPTH` / `HN_EVAL_MAX`）。
  **上限を書いたら流入と比べる**が守れているかを、コードの上で見えるようにするため。
- 各コレクタが「走査n件 → 条件合致n件 → 未収集n件 → 評価n件（▲最大〜最小）」を必ず出す。
  **数と一緒にスコアの範囲を出す**のは、次に並びが変わったときに数字で気づくため。

### 直さなかったもの

- **arXiv の窓は広げない。** 流入は1日数百件で、30件はその一部でしかない。広げると評価件数＝
  LLM課金が増えるので[[feedback-api-key-consent]]に従い別途GOを取る。いまは
  「arXiv＝量の面／HF daily_papers＝コミュニティの投票で選り分けた面」の二枚で受ける。
- **HFの下限（DL≥1000 or ♥100 / 👍≥5）はそのまま。** 今日は30/30が素通り＝実質効いていないが、
  収集ゲートは緩い側に倒すのが方針（[[pattern-filter-by-recoverability]]）。効かせる必要はない。
- **Papers with Code は触らない。** APIはHTMLを返していて実質死んでいるが、
  ソースは既に `status='stopped'` で回っていない。直す対象が無かった。

---

## 2026-09-13 ⑬ 同じ形を全工程で当たった（収集の外にも6件あった）

⑫で収集4本を直したあと「ほかにもあるのでは」と全工程を当たった。**あった。**
根拠はすべて `v2/backups/backup_2026-09-06.json`（21,900件）からの実測で、本番DBには触っていない。

### 測った土台
- **流入: 平均221.3件/日・ピーク318件/日**（直近30日）。重要度>=7 は **平均172.6件/日・ピーク251件/日**
- storyId 未割当 **0件** ＝ `runStoryGrouping(400)` は追いついている（健全の確認も実測で取る）

### 直したもの

| 箇所 | 何が起きていたか | 直し方 |
|---|---|---|
| `runEmbeddings` | 上限**300 < ピーク318件/日**。軽い日に回収できていたが、今日の増収集で余地が減る | 600へ。記事1件は一度しか埋め込まれない＝**上限を上げても総コストは増えない** |
| 月次レポート | `importance DESC LIMIT 100` が**★10だけで埋まり17日分**しか入らない。08-07〜08-20の**14日間が1件も入らないまま「月次」を書いていた** | `spreadByDay` で日ごとに配る。実測で **17日分→31日分・欠落0** |
| 週次レポート | 同じ形。日の欠落は無かったが最終日が **24/70＝34%** を占めていた | 同上（欠落0のまま均等化） |
| `translateTitles` | 新着**800行＝約3.6日分**しか走査せず、そこを出た未翻訳は永久に英語。実測**241件が窓の外**（うち93件は本物の英文） | 未翻訳を先に並べる。訳しようのない題（`llama.cpp.md` 等）は**原題を書き戻して印**にし、枠を占め続けるのを防ぐ（列の追加なし・表示は `titleJa \|\| title` なので不変） |
| `/topic` の relations | **ORDER BY 無しの LIMIT 20** ＝ rowid順＝最も古い20行。しかも重複除去は表示側。すぐ下の claims/benchmarks では同じ罠を直した記録が残っているのに、**この2本だけ直っていなかった** | SQLで `GROUP BY` して畳み、`MAX(id)` 降順で新しい関係から24件 |
| `/topic` の benchmarks / claims | 12件・8件取ってから表示側で「非ベンチ」「推測クレーム」を落とす＝**捨てる予定のもので枠が埋まる** | DBは40件・30件取り、**表示側で12件・8件に切る** |
| `/search` の重複除外 | 除外は上位25件なのに `/search` は**50件表示**。26〜50位が近傍に入ると同じ記事が二度出る | 除外を語彙一致の全件に広げる |
| `sitemap.xml` | 記事は**新着200件のみ**＝流入221件/日に対し**1日未満**。一覧の続きはクライアント側ボタンでクローラが辿れず、発見経路が事実上ここだけ | id と日付だけの専用クエリで5,000件（≒23日分）に。全件(約22,000)にするかは「薄い記事を大量にインデックスさせるか」の別判断 |
| `daily-report` の候補0件 | 朝刊が出ない最大の分岐が**無言の `return null`**（同じ関数の後半では空レポートを warn している） | warn を出す |

### 恒久対策: 滞留を毎日ログに出す

`reportBacklog()` を足し、`[Embed]` `[Chunk]` `[Story]` `[DeepExtract]` の各段が
**「未処理の残りn件（1回の上限m件）」**を必ず出すようにした。残りが上限以上なら ⚠ を付ける
＝「翌日も追いつかない」。**「n件処理しました」は健康の証拠にならない**（[[pattern-throughput-starvation]]
のチェックリスト4）。これで次に上限を割ったとき、実測を待たずログで分かる。

### 直さなかったもの（測って害が無かった）

- **要約の英語残り: 0件**。タイトルと同じ窓の問題を疑ったが、実測で1件も無かった
- **`runStoryGrouping` / `runDeepExtraction` / `runChunkEmbeddings(250)`**: 上限が流入を上回っている。
  ただしチャンクは重要度>=7のピーク251件に対し250で薄い ＝ 上のログで様子を見る
...
## 2026-09-13 ⑮ 商品の核が一般論だった（まず測るだけ入れた・A案）

「もうこの状態で世に出せるんか？（内容面）」に答えるため、**実物を2つ読んだ**。

### 読んだもの

1. **今朝の号（cernoval.com の実物）**: 5本・項目は「何が起きたか／なぜ重要か／**実務への影響**」・
   **出典行なし**・約4.7分。＝今日14:06に入れた修正（実務への影響を廃止し出典を入れる／3分の器）が
   効く前の版。今朝04:07のランは今日の修正より前のコードで回っている
2. **2026-09-07号（バックアップの実物・全文）**: **5,890字＝9.8分**（約束の3倍）。中身も:
   - ハイライト2位が**リーク情報**を出来事として扱っている
   - 「実務への影響」5本が全部「〜に注力する必要があります」型の同じ一般論
   - 📊カテゴリ別トピックに**Redditの質問スレ**（「700ドルの予算でGPU選定の**アドバイスが求められています**」）
   - **個人アプリの宣伝**をそのまま（ViDiary「動画サイズは80%削減される検証済み事実もあります」）

2 の4点は 09-12〜09-13 の変更で構造的に塞いだ（個人投稿除外／カテゴリ別トピック廃止／
一次情報を最低2本／1,800字の器）。**塞いだ結果が出るのは明朝04:07が初回。**

### 見つけた本丸: 「なぜ重要か」に中身の指示が1つも無い

`REPORT_SYSTEM_PROMPT` を読むと:
```
*   **何が起きたか**: 1〜2文。2文目は数字・製品名・組織名といった具体を出すために使う
*   **なぜ重要か**: 1文          ← 長さの指定だけ。何を書くかがゼロ
```
商品の核（cernere＝選り分けて、その理由を添える）の項目だけ、中身が指定されていない。
だからLLMは一般論を書く。

### 決定: 直す前に測る（Aだけ先に入れる）

本人の指示「先にA」。**出力は一切変えない。**`src/lib/why-specificity.ts`（テスト5件）で
「なぜ重要か」に固有名詞または数字が入っているかを毎号数え、1行ログに出す。
文そのものはログに出さない（判定と根拠の語だけ）。

**基準線（過去の実物で測定）**: daily 114号のうちこの形式で取り出せた74号・357項目で
**具体 36.4% / 定型句 44.8%**。直近21号に絞ると **100項目中35件＝35.0%**、
**21号中2号は具体が1本も無い**。＝**3本に2本が一般論**が今の実力。

- 代理指標であることを明記した。固有名や数字が入っていても中身が薄いことはある。
  **1日の値で結論を出さず、この基準線との前後比較に使う**
- `AI` `LLM` `API` のような毎回出るラテン語は固有名の証拠として数えない
  （数えると全部が「具体」になって指標が死ぬ）
- 直す案B（「なぜ重要か」に構造の縛りを入れる）は、この数字が明朝の実物で再現してから

**⚠ 案D（「なぜ重要か」を本紙の過去記事に接続）は今回も触っていない。**
キーワード一致が5本中3本で誤接続した実測があるため → [[cernoval-rebuild]]

---

## 2026-09-13 ⑯ 「焼き直し」は既に解決済み／「指示は効く」の対照が取れた

⑮の続き。コードは変えていない。**過去の実物で測っただけ**の記録。

### 焼き直し（同じ話の繰り返し）は解決済み

ハイライトの見出しを2-gram一致率で比べ、直近7号のどれかと 0.45 以上なら「同じ話」と数えた。
...
**2026-07-07 の鮮度修正（重要度順2-7日窓 → 前回以降の新着のみ）で 31% → 1% に落ちている**
（→ [[reports-unification]]）。07-07以降は 62号298本中5件＝**1.7%**、うち1件は本文に「（続報）」と
明示されている正しい続報。**同じ号の中での重複は 76号367本で0組。**

→ **ここは直す対象ではない。健全も実測で確認しておく。**

⚠ ただし残り4件のうち2件は 09-04「Nvidia、Hugging Faceを130億ドルで買収」と
09-05「NVIDIA、Hugging Faceを129億ドル超で正式買収」＝**報道の翌日に正式発表**の形。
2026-09-13 に入れた「[一次情報] と [報道] を混ぜて1つの項目にしない」は、
**この形の重複をむしろ増やす向きに働く**。明朝以降ここを見る。

### 対照実験: 指示のある項目だけ具体率が高い

同じ判定（固有名詞または数字を含むか）を、ハイライトの全項目に当てた（daily 76号・357項目）。

| 項目 | 具体率 | 平均字数 | プロンプトの指示 |
|---|---|---|---|
| （見出し） | 82.6% | 40字 | — |
| **何が起きたか** | **93.3%** | 135字 | **「2文目は数字・製品名・組織名といった具体を出すために使う」** |
| **なぜ重要か** | **36.4%** | 118字 | **「1文」＝長さだけ。中身の指示なし** |
| 実務への影響 | 35.9% | 119字 | 中身の指示なし |

**同じLLM・同じ号・ほぼ同じ長さ（118字 vs 135字）で、指示のある項目だけ 93.3%。**
指示のない2項目が揃って36%前後に並ぶ。n=357 なので偶然では説明できない。
＝**「具体を出せ」という指示は効く**。案B（「なぜ重要か」に中身の縛りを入れる）に勝ち目がある。

### それでも案Bは明朝の1回を見てから入れる

理由は1つだけ: 2026-09-13 に**「実務への影響」を廃止した**ので、
LLMがその内容を「なぜ重要か」に吸って、指示なしでも具体率が動く可能性がある。
**同じ日に同じ項目へ2つ変更を入れると、どちらが効いたか分からなくなる**
（[[pattern-coverage-is-not-outcome]] と同じ過ち）。明朝の号で 36.4% が再現するのを見てから入れる。

---

## 2026-09-14 ⑰ 本番停止（Turso読み取り拒否）と、答え合わせの結果

### A. 本番が止まっている（対応は本人にしかできない）

```
LibsqlError: BLOCKED: Operation was blocked:
SQL read operations are forbidden (reads are blocked, do you need to upgrade your plan?)
```

- `/api/health` → `{"status":"down","checks":{"db":"ng"}}`
- トップは HTTP 200 だが中身は「まだ出ていません」だけ＝**読者から見れば停止**
- パイプラインも 2026-09-14 04:56Z / 08:52Z の2回とも同じ理由で失敗

**Tursoの読み取り枠を使い切っている。**使用量の内訳はダッシュボード／CLIが要るので私からは見えない
（turso CLI 未導入）。**枠のリセットを待つか、プランを上げるかは本人の判断。**

⚠ これは `/api/health` を外部監視に登録していれば即座に気づけた種類の停止。
未登録のまま2回目（1回目は19日間気づかなかった）。→ [[vibecoding-audit]]

**私がやったこと（止血のみ）**: noindex のあいだ sitemap が記事22,000件を列挙しないようにした。
`revalidate=3600` で**毎時22,000行を読む**のに、noindex＝クロールさせない状態では何も生まない。
**これが枯渇の主因だとは言っていない**（測れていない）。主因かどうかに関わらず残す理由が無い。

### B. 昨日直した収集4本は全部動いた（09-14 03:07 のラン）

```
[HN] 走査150件 → 条件合致13件 → 未収集1件 → 評価1件（▲218〜218）
[GitHub] 取得50件 → 未収集3件 → 評価3件
[HF models] 取得30件 → 話題30件 → 重複除去27件 → 既出除外2件 → 未収集2件 → 評価2件
[HF papers] 取得30件 → 5票以上30件 → 未収集2件 → 評価2件
```
HNは**打ち切りを外して13件見えている**（予測どおり）。GitHub は77日間0件だったものが3件。

### C. 滞留ログが初日に1件見つけた

```
[DeepExtract] ⚠ 未処理の残り1043件 ≧ 1回の上限300件＝翌日も追いつかない（滞留）
[Embed] 未処理の残り0件（1回の上限600件）
[Chunk] 未処理の残り0件（1回の上限250件）
[Story] 未処理の残り0件（1回の上限400件）
```
本文抽出だけ滞留1,043件。ただし消化300/日 vs 重要度>=7の流入173/日＝**純減127/日で約8日で解消**。
**いまは上限を上げない**。DBが読み取り拒否の状態でDB仕事を増やす場面ではない。

### D. 「なぜ重要か」は改善しなかった → 案Bを入れた

```
[Report] なぜ重要か 5本: 具体0本 / 定型句3本 — 1:一般論 2:一般論 3:一般論+定型句 4:一般論+定型句 5:一般論+定型句
```

基準線36.4%に対し **0/5**。前日に「実務への影響」を廃止した交絡を疑って1日待ったが、改善しなかった。

⚠ **n=5 なので「悪化した」とは言えない**（基準線36.4%でも0本になる確率は10.4%）。
言えるのは「**改善していない**」だけ。だが⑯の対照（指示のある項目93.3% / 無い項目36.4%・n=357）と
合わせれば、**指示を書かない限り動かない**と判断してよい。

**決定: プロンプトの「なぜ重要か」に構造の縛りを入れる。**
```
*   **なぜ重要か**: 1文。**固有名詞または数字を1つ以上入れて**、その出来事が誰の何を変えるかを書く。
    「〜が重要です」「〜が求められます」「〜が期待されます」のような、固有名も数字も無い一般論は書かない
```
字数でなく**構造**で縛る（[[pattern-llm-cannot-count]]）。効果は次に朝刊が出た日の
`[Report] なぜ重要か` 行で測る。**DBが復旧しないと測れない。**

### ⑰-補足: 「通算0件」の16本を3つに分けた（2026-09-14・ネットワーク実測）

沈黙ソース監視が名指しした16本を、取れないのか・ただ静かなのかで分けた。

| 分類 | 本数 | 中身 |
|---|---|---|
| **robots.txt で恒久的に取れない** | **7本** | The Register ＋ GitHub `releases.atom` 6本。`politeFetch` が必ず空を返すので、**毎回 robots.txt を取りに行って終わり**。永久に0件 |
| 取れるが7日以内の新着なし | 6本 | calv.info / MS Research / Meta engineering / Cloudflare AI / **EleutherAI（最終2023年）** / Netflix。更新頻度が低いだけ |
| 取れて新着もあるのに0件 | 3本 | PyTorch(7日以内5件) / Raschka(1件) / Spotify(1件)。**原因不明** |

**決定: 7本を種リストから外す**（コメントアウトして理由を残す）。リリース情報が要るなら
GitHub API（`/repos/{o}/{r}/releases`）で取り直す。robots で拒否されるフィードをRSS経路に置いても永久に0件。

⚠ **既存のDB行は `status='active'` のまま**なので、DBが復旧したら `status='stopped'` にする必要がある
（読み取り拒否中なので今はできない）。

~~⚠ **3本目のグループは未解決**~~ → **2026-09-14 に解決。上の表そのものが誤りだった。**

`backups/backup_2026-09-13.json` の `sources.createdAt` を見たら、
**PyTorch(09-13) / Raschka(09-12) / Spotify(09-13) はバックアップ当日〜前日に登録**されていた。
存在して0〜1日のソースに「通算0件」も何も無い。**「産出0件」を実績と読んだのが間違い**で、
不具合ではなかった（同じ理由で「7日以内の新着なし」6本のうち5本も09-12〜13登録）。

ただし**本物の不具合が1件出た**: 9/13に足した2本が**既存ソースの重複**。
`pytorch.org/blog/feed.xml` は `pytorch.org/feed/`（**実績47本**）と件名もリンクも完全に同一で、
重複側は `filterUnseenUrls` で必ず0件になる。`blog.cloudflare.com/tag/ai/rss/` は**最新が31日前**で
7日窓に何も入らず、一般フィード `blog.cloudflare.com/rss/`（7日以内4本・実績6本）の方が強い。
→ 種リストを実績のあるURLに差し替え（AI関連度は下流のゲートで絞れる）。

**教訓: 「0件」は原因ではなく症状。母数（いつからあるのか）を見ないと、無実のものを犯人にする。**

---

## ⑱ 「3分で読める」は直近2号でようやく守れた（2026-09-14 実測・n=121）

DBは読み取り拒否のままだが、**`backups/backup_2026-09-13.json`（23,649記事・reports 313件）が
リポジトリにある**ことに気づいた。本番DBのほぼ全部なので、復旧を待たずに測れる。

`readingSeconds()`（表示側と同じ式・600字/分）で daily 121号を全部測った:

| | 値 |
|---|---|
| 3分(180秒)超 | **119/121号 = 98.3%** |
| 中央値 | **8分33秒**（約束の2.8倍） |
| 四分位 | 25% 7分45秒 / 75% 9分16秒 / 最大 12分37秒 |

直近14号の推移（ここが本題）:

```
09-01 7:48  09-02 8:50  09-03 10:00 09-04 9:26 09-05 7:23 09-06 8:52 09-07 9:10
09-08 8:59  09-09 7:58  09-10 7:07  09-11 4:43 09-12 4:53 09-13 2:36 09-14 2:28
```

**09-13号 1,749字 / 09-14号 1,664字**＝上限1,800字の内側。**121号で初めて約束を守れた2号**。
09-11 から効き始め、09-13 の「実務への影響の廃止＋構造の縛り」で確定した。

### ここから言えること / 言えないこと

- 言える: **字数上限を渡すのをやめて構造（文と項目の数）で縛る方針は効いた**
  → [[pattern-llm-cannot-count]] の裏づけが n=121 の前後比較で取れた。
- **言えない**: 2号では「安定した」とは言えない。8月以前が98%超過なので、
  数号でも戻ったら構造の縛りが効いていない日がある証拠になる。**毎日 checkBudget のログを見る。**
- 残る課題は逆方向: 2分28秒＝**3分の器の82%**。短くする戦いは終わったので、
  次は「空いた30秒に何を入れるか」になる（中身の密度＝「なぜ重要か」の具体性）。

### 手段としての発見

**バックアップは検証データとして使える。** 本番が落ちていても、
収集の当たり判定・滞留・号の長さ・文面の質は全部ここで測れる。
（同じ回で「タイトル未翻訳10,674件」が実際は160件＝**67倍の過大報告**だったことも、これで見つかった）

---

## ⑲ 焼き直しは「直っていない」ように見えて、実際は直っていた（2026-09-14・n=401）

⑱と同じバックアップで、号をまたいだ見出しの重複を bigram Dice で測った。

**途中の誤り**: 閾値0.6で数えたら 全体6.0% / 07-07以降2.1% / **直近21号5.0%** となり、
「最近また焼き直しが増えた」と判断しかけた。**これは閾値の置き方が根拠を持っていなかった。**

閾値スイープ（401本・直前7号との最大類似度）:

```
0.70→16本  0.65→19本  0.60→24本  0.55→29本  0.50→38本  0.45→47本  0.40→57本  0.30→79本
```

**膝が無い**（1.00の塊のあとは連続的に増えるだけ）＝この指標に「ここから重複」という線は引けない。
→ [[pattern-distribution-hides-content]]「分布のギャップは閾値の根拠にならない」。

**判断できるのは完全一致だけ**なので、正規化後の完全一致を数えた。**8本**。内訳:

- **7本は全て5月**（05-21〜05-27。「量子ビット…」が4号連続など）＝2026-07-07の作り直し前
- **1本は 07-26→07-27** で、見出しに**「（続報）」と明記**されている＝プロンプトが許している形

**結論: 07-07以降、判断の要らない焼き直しは実質ゼロ。直っている。** 直近21号の5.0%は
全て類似度0.6未満の「似ているが別の見出し」で、defect とは言えない。**修正は入れない。**

### ついでに見つけた（今は実害なし）
`daily-report.ts` の重複回避は前回号を `content.slice(0, 1200)` で渡している。
09-04号は6,092字だったので**先頭20%しか見ていなかった**（ハイライト3本目以降は対象外）。
ただし号が1,664字まで縮んだ今は1,200字で約7割を覆う＝**穴は自然に塞がりつつある**。
号がまた伸びたらここが効いてくるので、⑱の長さ監視と一緒に見る。

---

## ⑳ 収集の健全性を実測（2026-09-14）— 2件は健全、1件は本人判断待ち

`backup_2026-09-13.json` で確認。**いずれもコードは変えていない。**

### 健全 ①: robots ゲートは効いている
Reddit の記事は通算 **2,306本、最新が 2026-09-09**。robots ゲートを入れた **09-10 以降は0本**。
`Disallow: /` を無視して毎日巡回していた状態は、宣言どおり止まっている。

### 健全 ②: 一次情報の判定に誤検出なし
`isPrimarySource()` は URL 全体の `includes()` なので、原理的には
`?url=https://openai.com/...` のようなクエリで誤爆しうる。全件で照合したところ
**該当621件すべてホストも一致・誤検出0**。フラジャイルではあるが**実害が無いので触らない**（差分最小）。

### 本人判断待ち: robots 違反で取った 2,306本をどうするか
09-10 より前に取った Reddit の記事は、**いま公開UIの一覧・検索・トピックに出ている**
（`DIGEST_EXCLUDED_HOSTS` は朝刊の枠から外すだけで、サイトからは消さない設計）。
収集そのものが robots.txt に反していたので、**残すか消すかは第三条の判断**であり、
2,306行の削除は本番の破壊操作なのでこちらでは実行しない。**公開前に決めること。**

### ついでの訂正
ソース別集計を `sources.value` のドメインで出すと、`techdrip.net` のような
**URL型アグリゲータが「出所」として上位に出る**が、記事のURLは元媒体のもの。
「どのソースが集めたか」と「誰が書いたか」は別物。★9以上の集計は記事URLのホストで見ること。

---

## ㉑ 障害中に「無い」と言わない（2026-09-14・読者向け6面）

`actions.ts` は失敗時も `[]` / `null` を返す（fail-open）。表示は落ちないが、**読者には
「データが無い」と嘘をつく**。2026-06-19〜07-08 の19日間停止を誰も気づけなかったのと同じ形が、
読者の見え方にも残っていた。今回の読み取り枠切れで**実際に何が見えていたか**を本番で確認した:

| 画面 | 障害中に出ていた文言 | 実際 |
|---|---|---|
| トップ | 今朝の朝刊は、まだ出ていません | 休刊ではない。しかもCTA「集めた記事を見る」の先も空＝行き止まり |
| 記事を探す | 記事がまだありません | **23,649本ある** |
| トピック | まだトピックがありません | **1,846件ある** |
| 検索 | 「OpenAI」に一致する記事は見つかりませんでした | **検索の偽陰性**。しかも上に「0件」と断言 |
| /articles/26178 | 記事が見つかりません | **実在する**。404本文は「共有リンクの記事が古い場合もあります」＝**リンクのせいにしていた** |
| /reports/324 | レポートが見つかりません | 同上 |

**決定: 表示するものが無いと分かった後だけ `isDbReachable()`（`SELECT 1`）を1回呼び、
「無い」と「いま繋がらない」を言い分ける。** 正常時はこのクエリは走らない。
監視は `/api/health` のまま（HTTPステータスで落とす）＝ここは**読者への言い方**だけ。

### とくに重かった2点
1. **検索の偽陰性**。「無い」と言われた読者は二度と探さない。ここだけは断言してはいけない。
2. **個別ページの404はキャッシュされる**。`revalidate = 3600` なので障害中に生成された404は
   **DBが戻ったあとも最大1時間**配られる。`notFound()` を投げずに `OutageNotice` を返すことで
   その汚染も同時に消えた。

### 実機検証で2回助かった
- 文言だけ直したら、すぐ上の **「0件」** が残って矛盾していた（スクショで発見 → 0件時は件数を出さない）。
- 本文を直したのに **`<title>` が「記事が見つかりません」のまま**だった（curlで発見）。
  タイトルは共有カード・ブックマークに残る＝後から取り消せないので、ここも言い分ける。
**「直した」と言う前に本番を見る。** curlだけ／スクショだけ、どちらか片方では両方とも見逃していた。

## ㉒ バッチ抽出の返却を「位置」でなく「id」で受け取る（2026-09-14）

**決定**: Gemini Batch API へ投入する各リクエストに `metadata: { articleId }` を持たせ、
取り込み時は `InlinedResponse.metadata.articleId` で記事に対応づける。添字は使わない。

**理由**: それまで `responses[i]` ↔ `j.articleIds[i]` の**配列の位置だけ**が唯一の結びつきだった。
応答が1件でも落ちる・並べ替わると、以降が全部1つずれる。しかも取り込みは
`ingestKnowledge(..., { replace: true })` なので、**記事Aの抽出結果で記事Bの知識を置き換える**＝
記事Bの既存の関係・ベンチ・主張が消える。知識グラフは公開面に出るので、
ずれは例外を出さずそのまま嘘になる。同じ型は本番で既に2回起きている（要約6件中4件の入れ替わり／
HF・HN・RSSの並び順の誤読）。

**不採用**: ①件数一致チェックだけ＝入れ替わりを検出できない（メモリに実証済み）。
②LLMにidを復唱させる＝プロンプトとスキーマを増やすうえ、ハルシネーションの余地を作る。
`InlinedRequest.metadata` は **API の契約としてそのまま `InlinedResponse.metadata` に返る**
（`@google/genai` の型定義で確認）ので、モデルの協力もトークンも要らない。

**影響**: バッチモードは CI に無い手動実行（`PIPELINE_MODE=batch_submit` / `batch_fetch`）で、
投入中のジョブも無かった（`batch_state.json` 不在）ため、契約変更の影響を受けるものは無い。
metadata を持たない旧ジョブ用に、**件数が一致するときだけ**添字へ退避する経路を警告つきで残した。
一致しないときは取り込まずジョブごと中止する（`ingested` を立てないので再実行できる）。

## ㉓ 生バイトURLの500は自分のコードではない（調べて触らないと決めた・2026-09-14）

**症状**: `/category/研究` のように**パーセントエンコードしていない生のUTF-8**を含むURLが
HTTP 500 を返す（`X-Matched-Path: /500`・`X-Vercel-Cache: MISS` で再現）。
`/topic/研究` は404、`/category/é`（2バイト）は200。

**決定**: **直さない**。アプリのコードに原因は無いと判定したため。

**根拠**: `/articles/研究` も500になる。しかし middleware の `ID_PATH_RE` に当たれば、
`decodeURIComponent('研究')` は `%` を含まないので投げようがなく、次の `/^[1-9][0-9]{0,9}$/` に
落ちて **必ず 404 を返す**経路しかない。500になるということは、**middleware が実行される前**に
失敗している＝Next/Vercel のルーティング層の話で、こちらから手が届かない。

**やりかけて止めたこと**: middleware の記事/レポート枝は `decodeURIComponent` を素で呼んでおり、
トピック枝だけ try/catch で守られている。この非対称を「バグ」として塞ごうとしたが、
`/articles/%zz` `/reports/%E0%A4%A` を実際に投げると **400 が上流で返り**、不正な `%` は
そもそも middleware に到達しないことが分かった。＝**到達不能な箇所に防御を足すところだった**。

**実害の見積り**: ブラウザはパスを必ずパーセントエンコードするので、通常の読者は踏まない。
踏んだ場合も 500 が出るだけで、データや索引への影響は無い。

## ㉔ 焼き直し禁止の判定材料を「前号の本文1,200字」から「直近5号の見出し」へ（2026-09-15）

**症状（実物で発見）**: 同じ話が3号に出ていた。
09-09「OpenAI、ナビエ・ストークス問題の解法発表も一部で論争に」→
09-12「OpenAIのナビエ・ストークス方程式解決に不正疑惑」→
09-13「OpenAIの数学ブレークスルーが未発表データ利用で論争に」。
プロンプトには最初から「焼き直しは禁止」と書いてある。**指示ではなく材料が届いていなかった。**

**原因**: `prevDaily.content.slice(0, 1200)` ＝前回レポートの本文を先頭から1,200字だけ渡していた。
本文は**中央値5,690字**なので、**全83号401本のうち169本（42.1%）が最初から見えていない**。
09-12 のナビエ・ストークスは5本目＝上限の外。だから翌09-13 で1本目として返り咲いた。
（`pattern-throughput-starvation`＝上限が流入より小さく、静かに落ちる型の15+1件目）

**決定**: 本文を切るのをやめ、**直近5号のハイライト見出しだけ**を渡す（`buildRehashGuard`）。
字数上限に当たったら**古い号から丸ごと落とす**——途中で切ると、そこだけ欠けたのか
渡していないのかが後から分けられなくなるため。

**効果（backup_2026-09-13 で新旧を突き合わせ）**:

| | 旧 | 新 |
|---|---|---|
| 09-13生成時に「ナビエ・ストークス」が材料にあるか | いいえ | **はい** |
| 直近5号の見出し1,930本のうち材料に載った数 | 237本 (12.3%) | **1,906本 (98.8%)** |
| 1号あたりの平均字数 | 912字 | **733字** |

カバー率が8倍になり、**トークンはむしろ減る**。100%でないのは1,200字上限で最古の号が落ちるため（仕様）。

**不採用**: ①上限字数だけ増やす＝本文は中央値5,690字なので5号ぶんで約28,000字。毎日の費用に効く。
②直近1号のままにする＝09-09 と 09-12 は3日離れており、前号比較では構造的に捕まえられない。

**未確定**: 実際に焼き直しが減るかは次の号以降を見て測る（n=1では判断しない）。

## ㉕ 書いているのに誰も読んでいないデータ3件（2026-09-15）

スキーマの全16テーブルについて、定義以外からの参照数を機械的に数えた結果:

| テーブル | 行数 | 状態 |
|---|---|---|
| `alerts` | 1,722 | **参照0**。書きも読みもするコードが存在しない（07-07の私用機能撤去の取り残し） |
| `pipeline_logs` | 369 | INSERTのみ。**SELECTが本番コードに1件も無い** |
| `research_questions` | 326 | INSERTのみ（確信度低下のカスケード・1回最大5件）。**消費側は07-07に撤去済み** |
| `chat_memory` | — | 退会時のDELETEのみ。チャットは06-10撤去。**残すのが正しい**（過去行の消去動線） |

**やったこと**: `logPipeline` に直近10回の中央値を並べて出すようにした。
2026-09-13 18:49 の回が **2,531秒（直近の中央値173秒）** だったことに誰も気づいていなかった。

**しきい値による警告は入れなかった**。`pipeline_logs` は**どのモードで動いたかを記録していない**ため、
収集の回と夜間の重い回が同じ列に混ざっている。「中央値の3倍で警告」を全369回で試すと
**364回中109回（29.9%）鳴った**（09-07〜09-11 の21時台が毎晩2,200〜3,100秒＝別モードの正常値）。
3回に1回鳴る警告は読まれないので、判定せず数字を並べるだけにした。
モード列を足して初めて警告にできる＝本番スキーマ変更なので読み取り枠が戻ってから。
...
## ㉗ キーワード発見は「輪が閉じていない」まま毎日回っている（2026-09-15・警告だけ追加）

**構造**: キーワード候補が `active` になる道は `runEvolve` の昇格だけで、条件は「14日で3ヒット＋採用歴」。
ところがヒットは `collectData` が `status='active'` のソースしか巡回しないので発生しえない。
**＝候補 → active の輪が閉じていない。** 作られた候補は一度も使われず14日後に必ず `stopped` になる。

**実測（backup_2026-09-13）**:
- keyword型 697件のうち **active は0件**（stopped 656 / candidate 40 / low-priority 1）
- 候補40件の**採用歴は0件**
- 08-01以降の採用373件のうち keyword は **2件**（rss 282 / url 35 / hn 28 / arxiv 24）
- キーワード検索は毎朝10ラウンド回る設定だが、対象0件なので即 `break` している

**やったこと**: 警告を1本足しただけ。`[Evolve] ⚠ activeなkeywordソースが0件…` を毎回出す。

**やらなかったこと**:
- **キーワード発見のLLM呼び出しを止めること**。毎日1回 `gemini-2.5-flash-lite` を呼んで
  捨てる行を作っているが、止めるのは機能の撤去であり商品の判断（[[current-phase-plan]] の
  「検索0%の最終スイッチ」は撤去の方向だが、`measure_v4.ts` の再計測が前提条件のまま）。
- **`return` で以降を飛ばすこと**。キーワード発見の後ろには「高品質記事のドメインから
  RSSを自動発見する（検索の卒業エンジン）」と「重要度スコアのパーセンタイル正規化」があり、
  **どちらも現役**。ここを飛ばすと収集源の発見が止まる。
...
## ㉚ 公開面に数値として出ていた「ベンチマークでないもの」を落とす（2026-09-15）

**測ったこと**: `benchmarks` 504件に表示側と同じゲートを当てると383件が通過していた。
その383件の中身を全部読んだ。
...
| 形 | 件数 | 実物 |
|---|---|---|
| ラテン文字を1つも含まない名前 | **53件** | `四タスク平均評価者` `事例`=150% `サイズ` `命令追従` `平均所得税率` |
| エンティティ名＝ベンチ名（自己参照） | 3件 | `MMBench2 / MMBench2 = 427hours` `Claude Opus / Claude Opus = 4.6`（版番号） |
| %なのに100超 | 1件 | `Mercari / 事例 = 150%`（上の53件に含まれる） |

**決定**: 両方を `isValidBenchmarkName` に入れた。第2引数 `entityName` は任意なので
既存の呼び出しは壊れない。取り込み（保存前）・クリーンアップ・トピック表示の**4箇所すべて**に配線。
表示通過 383件 → **330件**（13.8%減）。索引に出るトピックは181→180件。

**なぜラテン文字で切れるか**: 日本語圏の実在ベンチもラテン表記（JGLUE / JMMLU / JCommonsenseQA）。
ラテン文字が混ざるだけの名前（`Hugging Face月間ダウンロード数`）は**落とさない**＝別の問題として残す。
消える53件を全部目視して、実在ベンチが1件も含まれないことを確認済み。

**残った課題（今回は触らない）**: 同じモデル×同じベンチで値が食い違う組が **17組/295組（5.8%）**。
`yi-lightning / Chatbot Arena (Elo): 10rank vs 0Elo`（順位をEloとして保存）、
`randomized yarn / BaBILong: 16000 vs 128000`（コンテキスト長をスコアとして保存）など。
どちらが正しいかは記事を読まないと決まらないので、機械的には片付かない。

## ㉛ トピックに文章として出る主張の点検（2026-09-15）

**測ったこと**: `claims` 3,328件 → active 3,109件 → 公開面に出る **1,168件**。無作為25件を読み、
そこから2つの形を特定して全件に当て、消える中身を全部目視してから採用した。

| 形 | 件数 | 実物 |
|---|---|---|
| 値が程度だけで中身が無い | 14件 | `Claude Opus 4.8 / コーディング能力 = 向上` `GPT-5.6 / 1ドルあたりの性能 = 向上` |
| 述語が動詞の活用で終わる＝文 | 51件 | `LLM推論を高速化する = ドラフトモデルを…`（値が説明文）／`AIモデルが顧客資産にアクセスした = GPT-5.6 Sol`（**値が主語**）／`…増加させる = これにより`（値が接続語） |

公開 1,168件 → **1,104件**。索引に出るトピックは **180 → 175件**（主張が痩せて「薄い」側に移る分）。

**すでにあったルールとの関係**: `isValidClaim` は「述語が40字超なら文」で長い文を弾いていた。
今回のは**同じ壊れ方の短い版**で、長さでは捕まらないものを形で捕まえる。

**⚠ 一度作って捨てた規則**: 最初は述語の末尾に `貢献|実現|回避|削減|提供|可能` も含めていた（62件該当）。
実物を読んだら **`KVCodec / TTFT（Time-To-First-Token）を最大削減 = 3.51倍`** のような
**正当な指標が消えていた**。これらはサ変名詞であって動詞の活用ではない。
落としてよいのは活用形（する/した/される/している/できる/させる…）だけ。この歯止めをテストに固定。

**やらなかったこと**: `OpenAI / 発表した = 未来のAI構想` のような行は、形としては文だが情報はある。
ただし claim は「subject / 指標 = 値」の形式なので、これは relations で表すべきもの。
抽出側の設計の話になるので今回は落とすだけに留める（作り直しはしない）。

## ㉜ 「関連トピック」が404と空ページへのリンクだらけだった（2026-09-15）

**症状**: `/topic/[name]` の「関連トピック」チップは `relations` の**自由文字列**を
そのまま `/topic/<名前>` のリンクにしていた。middleware は `entities` に無い名前を404にするので、
エンティティ化されていない相手は**押すと404になるリンク**として並んでいた。

**実測（関連に出うる938種）**:

| | 種類数 | |
|---|---|---|
| `entities` に無い＝**404** | 50 (5.3%) | `サムスン` `川崎重工業` `F#` `本書` `セブン-イレブン・ジャパン` |
| 在るが公開に耐えない＝薄いページ | 658 (70.1%) | `Amazon CEO Andy Jassy` `AI sheriff` `Hark` |
| 無事 | 230 (24.5%) | |

**決定**: 取得側（`getEntityKnowledgePage`）で `entities` と内部結合し、
`isPublishableEntity` を通った相手だけをリンクにする。上限は 24→60 に広げた——
先に24で切ると枠が使えない名前で埋まり、絞ったあと数件しか残らない
（[[pattern-throughput-starvation]]: 広く取る→絞るの順）。

**結合キーの選択（読み取り枠が切れている最中なので慎重に）**: `LOWER(名前)=LOWER(名前)` は
索引が効かず、関係1行ごとに `entities` を走査しかねない。`relations.object_entity_id` で結合すれば
整数の等価結合で済む。**両者が等価であることは実測で確認**した: 非stale 888件のうち
「idありだが名前がentitiesに無い」**0件**、「idはnullだが名前はentitiesに在る」**0件**。

**効果（backupで前後比較）**: チップ総数 749 → 470（63%）。
関連が丸ごと消えるトピックは 229件中62件（27.1%）だが、消えるのは**404か空ページ行きだったもの**。
索引に出る大きなトピックはほぼ保たれる（OpenAI 24→22 / Anthropic 24→19 / GPT5.5 17→13）。
[[feedback-subtraction]]「迷ったら足すより外す」。押して404になるリンクより、少なくても生きたリンク。

## ㉝ 記事の `#タグ` の3分の2が内部メタデータだった（2026-09-15）

**実測（backup_2026-09-13・タグ総数8,373件）**:

| 中身 | 件数 |
|---|---|
| `hn-score:770` のような内部キー:数値 | 681 |
| ホスト名（`techcrunch.com` `lobste.rs` `brew.sh`） | 1,728 |
| 収集元の名前（`hn` `arxiv` `zenn` `qiita`） | 3,199 |
| **話題を表しているもの** | **2,765 (33.0%)** |

**さらに悪い点**: 収集側は `[item.src, item.domain]` を**先頭に**入れる。記事ページは
`article.tags.slice(0, 6)` と先に上限を当てていたので、**6枠がメタデータで埋まり本来の話題タグが
押し出されていた**（[[pattern-throughput-starvation]] と同じ形）。ソース名は `sourceValue` として
別に表示済みなので重複でもある。

**決定**: 表示側に `isDisplayableTag` / `displayTags` を新設し、**絞ってから上限を当てる**。
記事詳細（6枠）と一覧（3枠）の両方に配線。**DBは変更しない**——`/tag/arxiv` のようなURLは
今までどおり開ける。止めるのは「記事の下に並べて読者に押させること」だけ
（[[pattern-filter-by-recoverability]]: 回復可能な側に掛ける）。

**誤爆の確認**: ホスト名判定にかかる685種の最終セグメントを全部見て、`next.js` のような
話題タグが1つも含まれないことを確認（`brew.sh` `lobste.rs` `ntsc.rs` は実在ホスト）。
将来のために `next.js` `llama.cpp` 等の既知の例外リストも持たせ、テストで固定した。

**⚠ 未検証**: これはUI変更なので本来は Playwright スクショで実機確認する
（[[feedback-visual-verification]]）。読み取り枠が切れていて記事が描画できないため、
**視覚確認は復旧後に持ち越し**。復旧後の確認事項に入れる。
...
## ㊱ 障害中のRSSが「更新日時つきの空フィード」を200で配っていた（2026-09-15）

本番がTursoの読み取り枠切れで止まっている状態を、**障害時の挙動を実機で測れる唯一の機会**
として使った。公開URL 11本を叩いてヘッダごと記録した。

**見つかった本物は1件。`/feed.xml` が HTTP 200・696バイト・`<item>` ゼロ。**
`getReportsData` は fail-open で `[]` を返すので、DBが落ちていても整形式のRSSが組み上がる。
そして `<lastBuildDate>` には「今」が入る。つまり**空のフィードに「たった今更新した」という
判子を押して配っていた**。欠落ではなく積極的な嘘で、購読者からは「Cernoval は何も出していない」
としか読めない。

さらにヘッダが `s-maxage=1800, stale-while-revalidate=86400` なので、
**障害中のたった1回の取得が、復旧後も最大24時間そのまま配られ続ける**。
画面の404と違って人間の目に触れないので、誰も気づけない。

対処: `reports.length === 0` なら 503 + `Cache-Control: no-store` + `Retry-After: 1800`。
公開レポートは313本あり0件になる正常系は存在しないので、0件＝こちら側の異常と断定してよい。
503ならRSSリーダは手持ちを保持して後で取り直す＝こちらが望む挙動そのもの。
`s-maxage/stale-while-revalidate` を付けて返すルートは他に無いことを `grep` で確認済み
（`api/health` は正しく `status:down` を返している）。

**自分の断定が2つ外れたので取り下げる。**

- **`sitemap.xml` が13件しか無いのを障害だと書きかけた** → `SITE_NOINDEX` が真のあいだ
  固定ページだけ返す仕様どおりで、入口6＋カテゴリ7＝ちょうど13。障害と無関係。
- **`/reports` が404なのをバグだと書きかけた** → 一覧ページは存在しない設計で、
  リンク元も `grep` で0件。404で正しい。

`/category/研究` の500は既知（㉓・生のUTF-8はmiddlewareより手前で500）。
`%E7%A0%94%E7%A9%B6` なら200で返ることを確認したので新規ではない。

**朝刊メールは無事**だった。同じ「fail-openで空のまま進む」型を疑って
`buildDailyReport()` を追ったが、候補0件で `null` を返し `runDailyReportAndDistribute` が
配信ごとスキップする。冪等チェックの `db.select` もDB障害では素直に落ちる。
一番取り返しがつかない経路（実在の購読者へ送る）に穴は無い。

### ❌ 自分のコードに書いた誤りを訂正した

`OutageNotice.tsx` に「notFound() を投げずにこの画面を返せば、（ISRの）汚染も起きない」と
書いてあったが**誤り**。障害中の `/articles/26178` を3秒おきに3回叩くと
`X-Nextjs-Prerender: 1` / `X-Vercel-Cache: HIT` のまま `Age: 79 → 82 → 86` と伸びた。
この画面も404とまったく同じようにISRキャッシュに格納されている。
**改善したのは文面だけで、キャッシュ汚染の範囲は1ミリも変わっていない。**
測らずに「起きない」と書いた断定だった。文面と実測をコメントに残した。

直し方は**あえて入れていない**。`connection()` はNext.js 16の同梱docsでは
「ビルド時プリレンダからの除外」が用途と書かれているだけで、実行時のISR格納を止めるとは
明記されていない。DBが戻るまで実機で確かめられないので推測で触らない
（AGENTS.md「コードを書く前に該当ガイドを読む」）。→ recovery-checklist.md 4章

Co-Authored-By 等は git 側に記録。

## ㊲ 記事の8.8%が「押すと別の記事に着地する」リンクを出していた（2026-09-15）

DBが読めないあいだ、バックアップで**読者が最初に触る面**を上から順に測った。
カテゴリ分布（7本とも946件以上あり空ページは無い）と要約の質（92.7%・中央107字・
Markdown混入0・英語のまま0・完全重複4件）は健全だった。**見出しと元記事URLで当たった。**

保存されたURLの **2,083件（8.81%）にHTMLエンティティが生のまま残っていた**。
`&#45;`（ハイフン）5,719回・`&#038;`（&）1,302回。内訳は gigazine.net 1,432件、
futurumgroup.com 651件。

`#` はURLのフラグメント開始なので、ブラウザは `#` より前だけを取りに行く。
**実際に叩いたら404ではなくHTTP 200が返り、無関係な記事が表示された。**

| | |
|---|---|
| 押した見出し | ジャック・ドーシーのBlockが開発者向けグループチャット「Buzz」をリリース |
| 実際に着地 | 「数千分の1秒で爆速動作するカメラのシャッター」をスーパースローで撮影すると… |

リンク切れなら読者は気づくが、これは**静かに間違った場所へ運ぶ**ので誰も気づけない。
しかも要約を出しておいて出典に飛べないので、第三条（要約＋元記事リンクで完結させる）に穴が空く。
[[pattern-positional-pairing]] と同じ型＝正しい見出しに間違った宛先を対応づけていた。

**取り込み側は既に直っていた。** 壊れたURLの最終収集は `2026-09-10 07:51:50`（エンティティ監査の
投入直前の回）で、それ以降に収集した gigazine 31件・futurumgroup 13件は**壊れ0件**。
つまり残っている2,083件は全部それ以前の在庫で、**DBに残り続けるだけで本番に出続けていた**。

対処は**描画の関所（`safeHttpUrl`）でエンティティを解く**ことにした。バックフィルを待たずに
デプロイした瞬間に2,083件全部が正しい宛先に戻る（読み取り枠が切れていてUPDATEできない状況でも効く）。
検査の**前に**解くので安全側にも倒れる（`&#106;avascript:` が本来の姿で検査にかかる）。
バックアップ23,647件で事前検証: リンクになる件数は 23,642 → 23,642 で**変化なし**、
文字列が変わるのが2,083件、**壊れるものは0件**。

### 関所が3本あった（第四条 DRY違反）

`safeHttpUrl` だけ直しても足りなかった。**まったく同じ判定式が `Markdown.tsx` と
`feed.xml/route.ts` に書き写されていた**ので、関所に入れた修正がその2本には効かない。
両方を `safeHttpUrl` の呼び出しに寄せた。1本見つけたら同型を全部当たる、の実例がまた出た。

DBの値そのものを揃える `scripts/fix_entity_urls.ts` も用意した（既定ドライラン・`--apply` 必須）。
急がない。描画で解いた時点で読者への実害は消えており、揃える目的は重複判定・検索・
エクスポートなど**描画を通らない経路**が生のURLを見るため。
解いた結果が既存行と衝突したら（＝同じ記事が2行）その行は飛ばす。統合はオーナー判断。

### 測って外れたもの（取り下げ）

- **「1,221行が一覧で二重に出ている」と書きかけた** → 突き合わせキーの作り方が誤り。
  `URL.pathname` を使ったので `#` 以降がフラグメントとして落ち、`gigazine.net/news/20260722&`
  に20件まとめて潰れていた。`youtube.com/watch?v=…` も141件が1件に潰れていた。
  **URL完全一致で測り直すと重複は0組**。取り込み時に一意化されている。
  皮肉なことに、この誤った測定が上のエンティティ混入を掘り当てた。
- **カテゴリの `CATS` が検証されていないのでは** → `z.enum(CATS)` と `includes` で
  取り込み時に強制済み。異物は3件（0.01%）で全部検証導入前の在庫。

## ㊳ 購読者に送った全メールで、本文に素の「*」が出ていた（2026-09-15）

朝刊の実物を購読者と同じ経路で描画して確かめた。`mdToLightHtml`（メール用の変換）を
`daily_pipeline.ts` から**そのまま切り出して**2026-09-14号を通した結果:
...
*   何が起きたか: AIシステムが経験とフィードバックから…
*   なぜ重要か: AIが自己を改善し続ける未来に向けた…
*   出典: arxiv.org [一次情報]
```

原因は、この変換だけが `^- ` しか箇条書きとみなしていなかったこと。
LLMは `*   ` を好んで出す。バックアップ実測で**公開143号のうち138号が `*` 記法**、
`-` だけなのは1号。しかも `*` を使った最初の号は **daily 2026-05-16 ＝ 1号目**なので、
**今まで送った全部のメール**が該当する。daily 122号の合計で3,216行。

**サイトとRSSは正しかった。** `components/Markdown.tsx` も
`app/feed.xml/route.ts` も `/^[-*] /` で両方拾えている。**メールだけが取り残されていた**。
同じ処理のコピーが3本あり、そのうち1本だけ古い、という第四条(DRY)の典型で、
㊲ の `safeHttpUrl` がやはり3本にコピーされていたのと**同じ日に2件目**が出たことになる。

対処: `src/lib/mail-markdown.ts` に切り出して `mailMarkdownToHtml` として公開し、
`daily_pipeline.ts` はそれを呼ぶだけにした。テスト7件を張ってある。
- 箇条書きは `[-*+]` + 空白。行頭の空白も6文字まで許す
  （5月の号は `    * **実践的ヒント:**` と字下げしており、許さない版では1,133箇所が残った）
- `---` も `<hr>` に（素の `---` が本文に1箇所出ていた）

143号すべてを通し直した検算: **素の記号 3,216行超 → 3個**（残りは2026-05-17の1号だけで、
さらに深い字下げ。既に送信済みで現行フォーマットでは出ない）。

⚠ `<p>` の中に `<h2>/<h3>` が入る不正なネストは**直していない**。4か月動いていて実害が
観測されておらず、直すには関数の構造ごと組み直すことになる（差分最小）。記録だけ残す。

### 朝刊から記事へ飛べない（オーナー判断が要る）

同じ調査で見つけたが、**こちらは勝手に直さない**。プロンプトの変更＝Gemini課金と
紙面の変更で、どちらも本人のGOが要るため（[[feedback-api-key-consent]] / [[feedback-proposal-go]]）。

- 公開143号の本文に `[ID:N]` は **0件**。外部リンクも10件だけ。
  出典は「`arxiv.org [一次情報]`」という**素のテキスト**で、リンクではない。
  つまり朝刊を読んだ人が記事に辿り着く手段がサイトにもメールにも無い。
- `[ID:N]` を描画するコードは3箇所（`Markdown.tsx` のボタン、`feed.xml` の `<a>`、
  `excerpt()` と `why-specificity.ts` のストリッパ）にあるが、これは **2026-07-07に撤去した
  `briefing` の残骸**。`[ID:N]` を使った号は briefing 16号（2026-05-26〜07-07）だけで、
  daily/weekly/monthly は**一度も使ったことがない**。
- 第三条は「公開UIは要約＋元記事リンク＋分析に限定」と定めている。朝刊は第三者の報道を
  要約しており、出典表示が素の媒体名だけという状態はこの線に届いていない。
- 提案の方向（GO待ち）: LLMにURLを書かせるのは捏造を招くので**させない**。素材に記事IDを
  添えて `[ID:N]` を出させ、**候補集合に実在するIDだけを通す決定論ゲート**を掛けて
  こちらでリンクに変換する。`[ID:N]` の描画は既にあるので、足すのは素材とゲートだけ。
  残骸コードを消すかどうかも、この判断とセットで決める（先に消すと二度手間）。

### 健全だったもの（測って問題なし）

- **発行の欠号**: daily 122日中121号（欠けたのは2026-06-18の1日で、既知の停止期）。
  weekly 17号・monthly 4号は間隔の乱れ0。同じ日に2号出ていたのは2026-06-11の1日だけで、
  冪等ガードを入れる前の事故。
...
## ㊵ RSSの一覧に素の「---」が出ていた／行の判定を1本に集約（2026-09-15）

㊲㊳で見つけた本物2件は、どちらも**利用者が見る成果物を実際に描画して眺めた**ときに出た。
データ表の監査では1件も出ていない。そこで同じ方法をRSSに当てた。
`markdownToFeedHtml` と `excerpt` を `feed.xml/route.ts` からそのまま切り出し、
一時ファイルに書いて tsx に解釈させ、**実際に配信される50件**（`MAX_ITEMS`）を通した。

本文HTMLは健全だった（`<li>` `<ul>` `<h2>` `<h3>` `<hr>` すべて出る）。
**`<description>` が壊れていた。**

`excerpt()` は `^[-*]\s+` で箇条書きの記号を落とすが、この規則は**記号の後に空白を要求する**ので
`---` が生き残る。その結果、RSSリーダの一覧に出る要約文に素の「---」が混ざっていた。
実測: **配信される50件のうち23件＝46%**。180字しかない抜粋の4字を無駄にしたうえ、
誤字のように見える。

### 同じ判定のコピーを増やさないために `src/lib/markdown-lines.ts` を作った

㊳で `mail-markdown.ts` の箇条書き規則を直したとき、**メールだけが字下げ入れ子を拾える**状態に
なり、サイト・RSSとの差を自分で作ってしまっていた。同じ型の事故を3回目にしないため、
「この行は箇条書きか / 水平線か」という**文法の判定だけ**を1本に出した。
描画そのもの（React要素 / HTML文字列 / インラインstyle付きHTML）は用途ごとに違ってよい。

- `BULLET_LINE` … `[-*+]` + 空白、行頭の空白は6文字まで。`g` フラグを付けない
  （`.test` と `.replace` の両方で使うので状態を持たせない。テストで固定した）
- `HR_LINE` … `---` `***` `___` `===` を3つ以上、その行だけ
- メールとRSSの両方をこれに寄せた。テスト9件。

検算（配信50件）: `<description>` の素の `---` **23件 → 0件**、
本文HTMLの行頭に残る素の記号 **6個 → 0個**。

### サイト（`Markdown.tsx`）だけ意図的に残した

`src/components/Markdown.tsx:98` の `/^[-*] /` はそのままにしてある。
**UI変更はPlaywrightスクショで実機検証する決まりで、DB停止中は記事もレポートも描画できず
撮れない**（[[feedback-visual-verification]]）。口頭で「直したはず」と言うために触らない。
影響は配信50件で6行・全部8月以前の号なので急がない。→ recovery-checklist.md

### 測って「問題なし」だったもの

- **「3分で読める」という商品の約束**: 全122号で測ると97.5%が3分超過だが、
  **これは旧フォーマットの履歴**。2026-09-13の構成変更（カテゴリ別トピック廃止・
  実務への影響廃止）のあとは 09-13が2.60分、09-14が2.47分で、予算の82〜87%を使えている。
  集計だけ見て「約束を守れていない」と報告しかけたが、**日付順に並べたら直っていた**。
  ⚠ n=2 なので「安定して収まる」とまでは言えない。復旧後も毎日見る。

## ㊶ 共有カードの77%が「🔥 今日のハイライト」だった（2026-09-15）

「利用者が見る成果物を実際に描画して眺める」の続き。まだ見ていなかった成果物＝
**X / Slack / はてブで共有したときに出るOGカード**を当たった。

`reports/[id]/opengraph-image.tsx` の `headlineOf` は**本文の最初の見出し**を主役に採る。
現行フォーマットの最初の見出しは `## 🔥 今日のハイライト` なので、カードにはそれが出ていた。
...
共有された相手には、その号が何の話なのか**1文字も伝わっていなかった**。
記事URLの実体参照（㊲）や朝刊メールの素の `*`（㊳）と同じで、
**誰も見ていない面だから誰も気づかない**という型である。

### 規則を3段にして `src/lib/report-headline.ts` に出した

1. `###` の見出しのうち、節のラベルでない最初のもの（＝その号の1本目の記事）
2. 無ければ全レベルの見出しのうち節のラベルでない最初のもの
3. 無ければ最初の見出し／それも無ければ既定値

`###` を先に見るのは、weekly が本文に `# AI技術動向 週次サマリーレポート` を持っており、
それを採るとカード上部の kicker（`週次レポート · 2026-09-13`）と**同じことを二度言う**ため。
段を1つにした案（「節ラベルでない最初の見出し」だけ）も試したが、まさにこの weekly が
本文H1を拾ってしまった。段を2つにした案（`###` 優先のみ）では古い7号が節ラベルのまま残った。
**3段にして初めて143号すべてが記事名になる。**

検証（143号）: 節のラベルのまま **110号 → 0号** / 空 **0号** / 変わる 139号 /
見出しの長さ中央39字・最大89字（`renderEntityOgImage` が84字で丸め、
文字数に応じて50/60/70pxに落とすので溢れない）。テスト7件。

### 実機で画像を焼いて目で見た

口頭で「直したはず」と言わないため、`renderEntityOgImage` を直接呼んで
**PNGを3枚焼いて確認した**（[[feedback-visual-verification]]）。
- 修正前: 「🔥 今日のハイライト」が1行で鎮座している
- 修正後: 「人間によって構築された最後のAI：真の再帰的自己改善に向けて」が2行に収まる
- 最長（weekly 2026-08-30・89字）: 4行＋`…` で丸まり、カード内に収まっている

記事側（`articles/[id]/opengraph-image.tsx`）は `titleJa` をそのまま使っており、
この問題は無い（見出しの選択をしていない）。

## ㊷ 構造化データの画像が全ページ同じアプリアイコンだった（2026-09-15）

「利用者が見る成果物を描画して眺める」を、**機械が読む成果物**にも当てた。
`articles/[id]` と `reports/[id]` の JSON-LD を読むと、`image` が
`${SITE_URL}/icon-512.png` ＝ **512×512のアプリアイコン**を指していた。全ページ同じものである。

Google の Article 構造化データは画像に**幅1200px以上**を求めており、512pxでは推奨を満たさない。
そのうえ「この記事の画像」としてアプリアイコンを申告しているので、内容とも対応していない。

**同じページに正しい画像が既にあった。** 本番で叩くと
`/articles/26178/opengraph-image` は 200 / image/png / 53,771バイト、
`/reports/324/opengraph-image` は 200 / 49,355バイトを返す。
しかも Next は `og:image` にこれを自動注入しており、本番HTMLには
`<meta property="og:image" content="…/opengraph-image?01851c…">` と `og:image:width = 1200` が出ている。
**SNSのカードは正しくて、構造化データだけが取り残されていた。**

対処: `image` を `{ '@type': 'ImageObject', url: <このページのopengraph-image>, width: 1200, height: 630 }` に。
`publisher.logo` はロゴなのでアイコンのままにした。

⚠ 実機の確認は復旧後（障害中は記事ページが障害画面になり、JSON-LDが出ない）。
→ recovery-checklist.md 4章

### 実物を描画して確認したもの

**購読者に届くメール全体**を組み立て、Playwrightで描画して見た（㊳の修正の視覚確認も兼ねる）。
箇条書きは `<ul>/<li>` に、水平線は `<hr>` に、見出しは色付きで出ており、**素の `*` は1つも無い**。

### 既知だったので「発見」として報告しないもの

メールのフッタが**2行しか無い**（配信停止＋プロフィール案内）ことを確認した。
`OPERATOR_NAME` / `OPERATOR_ADDRESS` / `CONTACT_EMAIL` が未設定なので、`mailFooter()` が
送信者名・所在地・問い合わせ先を静かに省いている。
**これは既に記録されている**（`daily_pipeline.ts:29` のコメント、decisions.md の
個人情報保護法32条の節、`privacy/page.tsx:207`）。新規の発見ではない。
ただし**公開チェックリストには載っていなかった**ので、そこへ集約した。
単なる env 入力ではなく [[privacy-identity-scrub]] の匿名化方針と正面からぶつかる判断なので、
オーナー判断として書いてある。

## ㊸ Push通知が毎朝まったく同じ固定文だった（2026-09-15）

㊶（共有カードが毎回「🔥 今日のハイライト」）と**まったく同じ型**が、もう1つの面にあった。
端末に届くWeb Push通知の中身は:

```
Cernoval
今日のダイジェストができました。最新のAI動向をチェック。
```

これが**毎朝そのまま**送られていた。この文には**今日開く理由が1文字も入っていない**。
通知欄に並んだとき、昨日のものと見分けがつかない（`tag: 'kt-daily'` で上書きされるので
実際には1つしか残らないが、それでも「何の話か」は伝わらない）。

v10で通知を作ったときの仮置きがそのまま残っていたもので、意図を説明するコメントも無かった。

対処: ㊶で作った `reportHeadline()` をそのまま使い、その号の1本目の見出しを本文にする。
`sendDigestPush(reportId)` に本文も渡すようにした（`result.text` は既に手元にある）。

- **LLMは使わない。** `reportHeadline` は本文から決定論的に抜くだけで、追加コストはゼロ。
- 本文が渡らない経路（DBから拾い直した場合など）は従来の文に落ちる。
- 端末側は2行程度で切るので120字の上限を掛けた。実測では**全122号が上限内**（中央37字・最大70字）。
- 見出しが取れず従来文に落ちるのは1号だけ（本文が空の号）。

実データでの見え方:

| 日付 | 通知欄に出る文 |
|---|---|
| 2026-09-14 | 人間によって構築された最後のAI：真の再帰的自己改善に向けて |
| 2026-09-13 | OpenAIの数学ブレークスルーが未発表データ利用で論争に |
| 2026-09-12 | OpenAIのサム・アルトマンCEO、AI開発速度を緩める可能性を示唆 |

⚠ これは**実在の端末に届く文面の変更**である。送信そのものは以前から毎朝走っており、
新たに何かを送り始めるわけではない（LLM課金も発生しない）。それでも紙面に準じる面なので、
気に入らなければ戻せるよう、ここに理由ごと残しておく。
...
## ㊺ サイト本体のレポート描画で、字下げされた箇条書きが強調ごと壊れていた（2026-09-15）

**決定**: `Markdown.tsx` の箇条書き判定 `/^[-*] /` + `line.slice(2)` を、
`markdown-lines.ts` の `BULLET_LINE` / `bulletContent()` に置き換える。

**理由（実測）**:
- 症状は「箇条書きの点が出ない」では済まない。行頭に残った `*` が `parseInline` の
  斜体パターン `\*[^*]+\*` と対になるため、**その行の強調が全部1つずつズレる**。
  実際の描画（id=321・2026-09-13 weekly）:
  元 `    *   **新登場**: GPT-6 Astraの人気が…、**OpenAIのサーバーが…**が浮上しました`
  → `<em>   </em><em>新登場</em><em>: GPT-6 Astraの人気が…</em><em>OpenAIのサーバーが…</em>`
  **太字にしたい語が普通の字になり、本文が丸ごとイタリックになり、「可能性*が浮上しました」と
  文中に `*` が残る**。スクショで前後を確認済み。
- 規模: `backup_2026-09-13` の公開142号で該当 **1,138行 / 68号（47.9%）**。
- 「古い形式の残骸」ではない。月別の率は 05月31.7%→09月7.3%と下がっているが**ゼロにならない**。
  直近14日でも 09-13 weekly(8行) / 09-08(8) / 09-05(6) / 09-04(9) / 09-03(8) / 09-01 monthly(2)。
  LLM出力のゆらぎなので、プロンプト側でなく**描画側で吸収する**のが正しい。
- これは㊳（メールの素の `*`）・㊵（RSSの素の `---`）と**同じ形の3つ目**で、最後のコピー。
  → [[pattern-positional-pairing]]「1本見つけたら同型を全部当たる」

**不採用**:
- 番号付きリストの字下げ対応（同じ形だが実測 **3行 / 1号**しかなく、しかも `1.` は `*` と違って
  強調の対をズラさないので被害が桁違いに小さい。測った結果として見送る）。
- `feed.xml` の `excerpt()` に残る `/^[-*]\s+/gm`。字下げ行を拾えないのは同じだが、
  最後に `.replace(/[*\`>]/g, '')` が全アスタリスクを落とすので**抜粋に実害が無い**ことを確認した。
- `___` / `===` の水平線表記の統一（公開142号での出現が **0行**）。

**影響**: 過去68号の表示がその場で直る（レポート本文はDB側を触らない）。
回帰テストを `src/lib/markdown-render.test.ts` に4件追加（275件）。
`***` が箇条書きに吸われないことも同時に固定した。

## ㊻ 全142号を「描いてから」読んで、残っていた生Markdownを全部潰した（2026-09-15）

**決定**: 公開142号をサイト本体（`Markdown.tsx`）と購読者メール（`mail-markdown.ts`）の
**実際の描画関数に通し**、読者の目に記号が残る行を総当たりで洗い出して塞いだ。

**やり方**: データ表を眺めるのではなく、実物を描画して plain text に落とし、
`*` / `#` / `](` / `` ` `` が残る行を数えた（`scripts/_probe_leak2.ts`）。㊺までと同じ方法。

**見つかったもの（すべて実測）**:

| 経路 | 症状 | 規模 | 最後の発生 |
|---|---|---|---|
| サイト | `#### LLM推論` が記号のまま | 10行 / 2号 | 2026-08-04 |
| サイト | 空白8個の字下げ箇条書きを取りこぼす | 3行 / 1号 | 2026-05 |
| サイト | 太字の中のコード `` `torch.compile` `` が記号のまま | 1行 / 1号 | 2026-05 |
| メール | `#`（週次/月次の表題）と `####` が記号のまま | 24号 / daily 121号（19.8%） | 2026-08-12 |
| メール | `[タイトル](URL)` が記号とURLのまま | 10行 / 1号 | 2026-05-18 |

**直した内容**:
- `mail-markdown.ts`: 見出しを**レベルごとの正規表現でなく1本の規則**（`/^(#{1,6}) +(.+)$/gm`）に統一。
  レベルを足し忘れるのが原因だったうえ、順序の罠もある（`^# ` を `^## ` より先に置くと
  `## X` が「# X」として食われる）。あわせて `[text](url)` を `safeHttpUrl` 経由でリンク化。
- `Markdown.tsx`: `#{4,6}` の見出し分岐を追加。太字の中身を `parseInline` で再帰的に解釈
  （`[^*]+` の内側に `*` は入らないので再帰は必ず浅く終わる）。
- `markdown-lines.ts`: `BULLET_LINE` の字下げ上限 `{0,6}` を**撤廃**。

**❌ 訂正**: ㊺で `BULLET_LINE` に付けた上限6文字には**根拠が無かった**。
理由に挙げた「深く字下げしたコードブロックを吸い込まないため」は、公開142号で
フェンス(```) が **0行**＝守る対象が存在しない。しかも項目数を**数える側**
（`daily-report.ts` の `isBullet` = `^\s*[*-]\s+`）は最初から字下げを無制限に許していた。
**数える側と描く側で「箇条書き」の定義が食い違っていた**のが、字下げ行が構造ゲートを
通過したのに描画で壊れていた根っこ。定義を数える側に揃えた。
旧ルールを固定していたテスト（「深すぎる字下げは吸い込まない」）も差し替えた。

**言えること / 言えないこと**:
- サイト側は**遡って直る**（過去号の表示がその場で正しくなる）。
- メール側は**すでに送った分は直らない**。価値は今後の号に対してのみ。
  それでも直す理由: `enforceStructure` は見出しレベルを正規化せず素通しするので、
  `#` も `####` もいつでも再発しうる。実際、字下げ箇条書きは 2026-09-13 の号で再発していた（㊺）。
- 最終確認: 修正後に同じ総当たりを回して、**両経路とも142号で残り0行**。

**不採用**: この総当たりをテスト化する案。バックアップは固定スナップショットなので
テストにしても hold-out にならず（[[pattern-tests-from-observed-failures]]）、50MBの読み込みで
スイートも重くなる。probe スクリプトとして残し、代わりに観測した型ごとのユニットテストを6件追加（281件）。

## ㊼ トップ（朝刊の紙面）にも生Markdownが出ていた — 描画は3経路あった（2026-09-15）

**決定**: 紙面用の整形 `cleanText`（`digest-highlights.ts`）で、`**` に加えて
**コード記法とリンク記法も中身だけ**にする。

**理由**: ㊻でサイト記事本文とメールを掃除したが、**トップは第3の経路**だった。
`/`（今朝の朝刊）と `/about` は `Markdown.tsx` でも `mail-markdown.ts` でもなく、
`parseDigest` → `DigestBody` で描かれる。理由はファイル冒頭に書いてあるとおり
「紙面は黒白のバンドという別の視覚世界で、構造だけ返して色は紙面側が決める」ため。
その整形が `**` と `(ID:n)` しか落としていなかった。

**実測（公開142号・読者に出る文字列 10,227本を検査）**:
- バッククォート **56本 / 13号**。最後は **2026-09-04**（5月・7月・8月・9月と断続的に出る＝現行）。
  例: 「PyTorch 2.5の最新機能（特に`torch.compile`とC++バックエンド）を活用する」
- リンク記法 **10本 / 1号**（2026-05-18）。例: 「参考: [PyTorch 2.5 Release Blog](https://…)」
  うち1本は `vertexaisearch.cloud.google.com`＝`safeHttpUrl` が拒否する中継URLで、
  **死んだURLが素のまま紙面に出ていた**。

**なぜリンクを `<a>` にしないのか**: 紙面は意図的にインライン装飾を持たない（`**` も落としている）。
リンクを張るのは商品の見せ方の変更なので、ここでは**ラベルだけ残す**に留める。
記事本文ページ（`Markdown.tsx`）は `<code>` と `<a>` にする＝**経路ごとに違ってよい**が、
「記号を読者に見せない」という結論は3経路で揃える。

**確認**: 修正後に同じ総当たりを回し、**サイト・メール・紙面の3経路すべてで142号・残り0本**。
テスト2件追加（283件）。

⚠ 実機スクショは撮れていない。`DigestBody` はCSSモジュールを含むので単体描画できず、
本番はDB障害で朝刊自体が出ない。代わりに**読者が見る文字列の前後**を実データで示した
（`scripts/_probe_cleandiff.ts`）。復旧後に紙面のスクショを撮ること → recovery-checklist。

## ㊽ 描画経路は4つあった — RSSはテストから呼べない場所にいたので漏れていた（2026-09-15）

**決定**: RSSの本文組み立て（`esc` / `inlineHtml` / `markdownToFeedHtml` / `excerpt`）を
`app/feed.xml/route.ts` から `src/lib/feed-markdown.ts` へ出し、テストを6件張る。
あわせて `#{4,6}` の見出しと、太字の中身の再帰解釈を追加。

**理由**: ㊻㊼で3経路を掃除したが、**RSSが4つ目だった**。しかもこれは
「見落とした」というより**構造的に見えなかった**: 純粋関数がルートのファイルの中にあり、
`npm test` の対象（`src/**/*.test.ts`）からも、外から import する総当たりプローブからも呼べなかった。
Next のルートファイルはHTTPメソッドとセグメント設定を出す場所なので、
検証したい純粋関数を同居させると**その経路だけ検査から外れる**。
メール（㊳で `mail-markdown.ts` に出した）とまったく同じ形。

**実測（公開142号のRSS本文）**:
- `#### LLM推論` が記号のまま `<p>` に落ちていた: **10行 / 2号**
- 太字の中のコード `` `torch.compile` `` が記号のまま: **1行 / 1号**
- 修正後は **残り0行**。

**4経路の現在地（すべて142号・残り0行を確認）**:

| 経路 | 実装 | インライン装飾 |
|---|---|---|
| 記事/レポート本文ページ | `components/Markdown.tsx` | `<strong>` `<code>` `<a>` |
| 購読者メール | `lib/mail-markdown.ts` | インラインstyle付きHTML |
| トップの紙面・/about | `lib/digest.ts` + `DigestBody` | **持たない**（記号は落として中身だけ） |
| RSS | `lib/feed-markdown.ts` | 素のセマンティックHTML（白背景前提） |

**描画は経路ごとに違ってよい**が、①「この行は何か」の判定（`markdown-lines.ts`）、
②リンクの安全化（`safeHttpUrl`）、③「記号を読者に見せない」という結論、の3つは揃える。

**副産物**: 移動して初めて張れたテストに、**フィードへのタグ注入を防ぐエスケープの確認**を入れた
（`<script>` が `&lt;script&gt;` になること・`esc` の5文字）。第一条の砦がテストで固定されたのは今回が初めて。

**不採用**: 4経路を1つの描画関数に統合する案。出力先の制約が本当に違う
（RSSリーダは白背景・メールは外部CSSを落とす・紙面は黒白のバンド）ので、
統合すると各所に分岐が生えて今より読みにくくなる。揃えるのは判定と結論だけでよい。

## ㊾ 過去121号のうち33号は、トップの紙面として開けていなかった（2026-09-15）

**決定**: `parseHighlights` に、`### ` で1本も割れなかったときの**後方互換**を足す。
行頭の箇条書きを1本、字下げした箇条書きをその要点として読む。

**見つけ方**: 「🔥ハイライト節を持つ号」と「実際にハイライトが取れる号」を別々に数えたら食い違った。
節が無いのは **5号**だけなのに、0本になるのは **38号**。差の **33号が黙って落ちていた**。
1つの数字だけ見ていたら「そういう号もある」で終わっていた。

**実測（daily 121号）**:

| | 修正前 | 修正後 |
|---|---|---|
| ハイライト0本 | **38号** | 5号（本当に🔥節が無い5月の号） |
| 5本 | 70号 | **90号** |
| 何らかのハイライトを持つ | 83号 (68.6%) | **116号 (95.9%)** |

**原因**: 2026-05-28〜2026-07-07 の生成側は、各ハイライトを `### 見出し` ではなく
`*   **見出し**` の行頭箇条書きで出していた。`parseHighlights` は `^### ` でしか割らない。

**なぜ直す価値があるか（部分的な失敗ではなく全体的な失敗だから）**:
- 0本になると `parseDigest` はハイライトを返せず、トップも過去号ページも**紙面の組みを失って生の塊に落ちる**。
  ㊺〜㊽の記号の残りが「1行が汚れる」だったのに対し、こちらは**1号まるごと**別物になる。
- そして再発しうる。🔥の構造ゲート（`daily-report.ts` の `STRUCTURE`）は
  `bulletSentences` しか見ておらず、**見出しレベルを強制していない**。
  `#`/`####`（㊻）や字下げ箇条書き（㊺）と同じで、形式は約束であってゲートではない。

**あわせて**: 同ファイル内にあった5本目の箇条書き判定 `^\s*[*\-+]\s+` を `markdown-lines.ts` に寄せ、
ラベル解釈を `toPoint()` に切り出して新旧2経路で共有した（ここでコピーを作ると同じ穴が空く）。

**不採用**: 生成側のプロンプトを直して形式を強制する案。プロンプト変更は Gemini 消費と編集方針の
変更を伴うので本人のGOが要る（[[feedback-api-key-consent]]）。**読む側を頑丈にするのはコスト0**で、
過去号にも遡って効く。ゲートを足すかどうかは別途提案する。

**確認**: テスト3件追加（292件）。旧レイアウト・新レイアウト・🔥節が無い場合の3つを固定し、
「次のセクションの本文を吸い込まない」ことも確かめた。

## ㊿ 一覧の「要約がない理由」が、こちらの取得失敗を元サイトのせいにしていた（2026-09-15）

**決定**: `noSummaryReason` の戻り値に `short` を加え、詳細版と一覧版の文言を**同じ分岐で決める**。
`noSummaryShort` は分岐を持たず `r.short` を返すだけにする。

**理由**: 分類する switch（`noSummaryReason`）と短文を作る switch（`noSummaryShort`）が
**別々に並んでいて、片方だけ古かった**。`not_read` は
①取得を試みて失敗した（http_403/404/429・timeout・fetch_error・too_short）と
②本文が無いと分かっている、の2つを含むのに、短文は②の文言に固定されていた:

> 本紙未読 — **元サイトが本文を配信していません**

403やタイムアウトは**こちらが取れなかった**だけで、相手は配信している。
404の本文が「共有リンクの記事が古い場合もあります」と読者のリンクのせいにしていたのと同じ型の誤り
（[[OutageNotice]] の経緯を参照）。一覧・検索・保存済み・パレットの4画面がこの短文を使っている。

**規模（backup_2026-09-13 実測）**: 要約なし **1,718件 / 23,649件（7.3%）**。うち
`unsupported_domain` 231 / `too_short` 40 / `http_403` 39 / `robots_disallow` 28 /
`not_html:*` 8 / `http_404` 3 / `timeout` 2 / `http_429` 1 / `fetch_error` 1 / エラー記録なし 1,365。
**誤った文言が出ていたのは86件**（0.36%）。数は小さいが、内容は**第三者について事実でない断定**なので直す。

**測って問題なしとしたもの（同じ調査の中で）**:
- 要約の欠落率は 05月9.9% → 09月5.3% と下がっており、6行超の要約は **0件**、長さも中央値107字。
- `hasBody` と `extractError` は `COLLECTED_SELECT` に**ちゃんと入っていた**＝5分類は一覧でも機能している
  （[[pattern-wired-but-never-called]] を疑ったが外れ。取り下げる）。
...
## 53 法務ページが、存在しない「下記の窓口」を案内していた（2026-09-15）

**決定**: 窓口の呼び名を `site.ts` の `CONTACT_POINT` に1本化し、privacy / terms の4箇所すべてで使う。

**理由（本番実測）**: `CONTACT_EMAIL`（`NEXT_PUBLIC_CONTACT_EMAIL`）は本番で未設定。
コードは未設定時に「下記の窓口」→「運営者」へ切り替える作りだったが、**4箇所のうち3箇所が
切り替えを忘れていた**。その結果、本番の文面はこうなっていた:

- `/privacy`「退会はサイト上でいつでも実行でき、その他の請求は**下記の窓口**で受け付けます」
- `/privacy`「…**下記の窓口**で受け付けます。ご本人であることを確認のうえ…手数料はいただきません」
- `/terms`「記述に事実と異なる点を発見された場合は、**下記の窓口**までご連絡ください。
  確認のうえ、速やかに訂正または削除します」

いずれも**その下に窓口が無い**。個人情報の開示・訂正・利用停止、GDPRの権利行使、
掲載内容の訂正依頼——**すべて約束している動線**なので、辿れないまま置いてはいけない。
同じ判断が4箇所に散っていたのが原因なので、1本にして書き分けを忘れられないようにした。
構造テストも追加（`src/` に「下記の窓口」の直書きがあれば落ちる）。

**⚠ これは文面の辻褄を合わせただけで、窓口が無い事実は変わらない**。
`privacy/page.tsx` の中には以前から
「運営者の氏名・住所をサイトに常時掲示する義務は無い。ただし『求める』ための窓口は要る
＝ CONTACT_EMAIL が未設定だと、この節は義務を満たせない」と書いてある。**まだ満たせていない。**

**測って分かった周辺の事実**:
- `/feedback` は HTTP 200 で存在するが、本文は「現在フィードバック窓口を準備中です。」だけ
  （フォームも mailto も無い）。しかも**トップ・記事一覧・about・トピックのどこからもリンクされていない**
  （`href="/feedback"` の出現数はいずれも0）。窓口の env が未設定だと導線ごと隠れる作りのため。
- `/feedback` は sitemap には**入っていない**（placeholder を検索エンジンに出してはいない）。
- ログイン済みユーザーの**退会（アカウント削除）は自己完結で動く**ので、削除の動線だけは生きている。
  無いのは「非ログインの第三者が運営者に連絡する手段」。

**本人にお願いしたいこと（コードでは解けない）**: `NEXT_PUBLIC_CONTACT_EMAIL` を設定するか、
Googleフォーム（`FEEDBACK_FORM_ACTION` / `FEEDBACK_ENTRY`）を用意する。
匿名性方針（[[privacy-identity-scrub]]）との兼ね合いがあるので判断は本人。
専用の転送アドレスなら身元を出さずに窓口だけ作れる。

**測って問題なしとしたもの（同じ調査）**: 未参照の export を機械的に洗って14件出たが、
- `awaitCrawlDelay` は**同一ファイル内の `politeFetch` から呼ばれていた**（検出器が同一ファイルを除外していただけ）。
- `isDigestExcluded` は未使用だが、**除外ルール自体は `DIGEST_EXCLUDED_HOSTS` としてSQLで効いている**
  （`daily-report.ts:43`）。
- `FeedbackModal` は `/feedback` ページに置き換えられた旧実装。
いずれも機能の欠落ではない。取り下げる（残骸の削除は 52 の提案に含める）。


54. **障害画面が「実在する記事は無い」と1時間言い続けていたのを、middlewareで止めた（2026-09-15）**
    — 2026-09-14 の読み取り枠切れで `/articles/[id]` と `/reports/[id]` が `OutageNotice`（「こちらの
    不具合です」）を返していたが、このルートは `revalidate = 3600` のISRなので、**その障害画面自体が
    キャッシュに格納**されていた。本番実測で `X-Nextjs-Prerender: 1` / `X-Vercel-Cache: HIT` のまま
    `Age` が 79→82→86 と伸びるのを確認。つまり**DBが戻ったあとも最大1時間、実在する記事に
    「お見せできません」を配り続ける**。読者から見れば「壊れている」と言われた記事が、直った後も
    しばらく壊れたままに見える。
    決め手は**測り方**だった。「DBを落とせないから実機で確かめられない」と書いて保留していたが、
    **枠切れ中の本番DBは既に落ちている**。`next build --webpack` → `next start` をそのまま繋げば
    ローカルで障害が再現し、しかも `.next/server/app/articles/26178.html` というファイルの有無で
    汚染を直接見られる。プレビューデプロイもenvフラグも要らなかった。
    その環境で候補を2つとも潰した。① `await connection()` は ISRルートの中では呼べず 500
    （同梱docsの「ビルド時プリレンダの除外」以上のことはしないという疑いが当たった）。
    ② `throw` はキャッシュこそ書かれない（docs の「Handling uncaught exceptions」どおり）が、
    ISR生成中の例外は `error.tsx` に落ちず**素の "Internal Server Error"（21バイト）**になる。
    つまり**ISRルートの中で障害を描く限り、キャッシュ汚染か白画面かの二択**になる。
    採ったのは「ISRルートに入る前に逃がす」。`src/middleware.ts` の `exists()` はDBエラーを
    握り潰して `true`（fail-open）を返していた＝**「聞けなかった」を「在る」と言い換えていた**のが
    汚染の入口だったので、`yes | no | unknown` の3値にし、unknown は `/outage`（force-dynamic）へ
    rewrite する。実測: `/articles/26178`・`/reports/324`・`/topic/openai` の3ルートとも
    `Cache-Control: no-store` の34KBの画面が出て、ISRファイルは生成されない。DBを引かない404の枝
    （`/articles/abc` 等5種）も 404/26,564バイトのまま無傷。
    **不採用**: ルートごとの `error.tsx`（ISR生成中は描かれないので実質デッドコード）。
    **交換したもの**: middleware はキャッシュより手前で走るので、障害中は既にキャッシュ済みの記事も
    障害画面になる。ただし `revalidate = 3600` なのでその利点は**障害開始から最大1時間しか存在しない**
    （2026-09-14からの障害では24時間以上経っており、とうに消えていた）。防ぐ汚染は障害中ずっと＋
    復旧後1時間なので、差し引きで交換する価値がある。
    **残り**: `/outage` は rewrite なので HTTP 200。本来 503 が正しいが、App Router の rewrite で
    ステータスは変えられない。サイト全体が noindex の現状では実害がないので据え置く。

55. **朝刊の選別上限に「残り何件か」を出すようにした（2026-09-15）**
    — メモリに「throughput starvation の恒久対策 = `reportBacklog()` で毎日『残りn件（上限m件）』」と
    記録されていたが、**その名前の関数はリポジトリのどこにも無かった**。「上限<流入で静かに欠落」は
    本番で計15箇所（処理側5・収集側4・工程全体6）見つかった型で、月次レポートは★10だけで枠が埋まって
    14日分が欠落し、sitemapは1日未満しか載っていなかった。どちらもエラーにならず、出力を数えるまで
    誰も知らなかった。その恒久対策自身が「実装済みのつもりで呼び出し0件」になっていた＝
    wired-but-never-called が対策そのものに刺さっていた形。
    **先に測った**（バックアップ60日分・決めつける前に）:
      - `CANDIDATE_LIMIT = 120` … **60日間で一度も効いていない**（候補数 最小10 / 中央36 / 最大63）
      - `TOP_N = 40` … 12/60日で到達。ただしこれは仕様どおり（40件は「LLMに渡す材料」であって
        掲載本数ではない。約束しているハイライトは5本で、最も薄い日でも22本ある）
    つまり**いま枯渇はしていない**。問題は、この余裕が無くなった日に誰も気づけないこと。
    そこで `src/lib/backlog.ts` に `formatBacklog()` を置き、`daily-report.ts` から毎日1行出す。
    `got >= cap` は「既にこぼれている」、`got >= cap * 0.8` は「もうすぐこぼれる」。
    **判断はせず数字を出すだけ**にした（閾値で自動停止させると、仕様どおり埋まる TOP_N で毎日鳴る）。
    再発防止は機械検査にした: `backlog.test.ts` が `daily-report.ts` の中に `formatBacklog(` が
    在ることを確かめ、さらに**呼び出しを消した写しでは検出器が落ちること**も同時に確かめる
    （常にtrueを返すだけのテストが緑になるのを防ぐ）。実ファイルの呼び出しを一時的に消して
    テストが赤くなることも確認済み。
    **副産物（この過程で測って全部外れた仮説・再調査しないこと）**:
      - `CATEGORY_COLORS` の網羅性 … 23,649件中の漏れは3件のみ（`マルチモーダル`2・`工具/框架`1）
      - `titleJa` 欠落45.1% … 翻訳漏れではなく**元から日本語の記事**（Qiita/Zenn/ITmedia等）。
        `titleJa || title` は正しい
      - 広告記事の混入 … 明示マーカー付きは23,649件中**1件**、ai_relevance=1、公開号への混入0件
      - `/reports/[id]` から非公開種別（corpus_health 110件・briefing 50件）が読めるか …
        `getReportById` が `type IN ('daily','weekly','monthly')` で正しく閉じている
      - robots.txt の `Allow: /` がクロールを招いて読み取り枠を食っているか … `site:` 検索が
        演算子を無視した無関係な結果（別サイト）を返すため**インデックス状況を測れない**。
        記録済みの判断（Disallowにすると noindex を読まれなくなる）を覆す根拠が無いので撤回

    ❌ **訂正（2026-09-15・同日）**: この項目は当初「恒久対策の `reportBacklog()` は実装されていなかった」
    という見出しで書いていたが、**それは誤りだった**。`reportBacklog()` は `v2/daily_pipeline.ts:2742` に
    実在し、`[Embed]` `[Chunk]` `[Story]` `[KeyPoints]` `[DeepExtract]` の**5箇所から呼ばれている**。
    誤った原因は**探し方**で、`grep -rn reportBacklog src/ scripts/` しか掛けていなかった。
    パイプライン本体は `v2/daily_pipeline.ts` にあり、**src/ にも scripts/ にも入っていない**。
    ⇒ このリポジトリで「呼び出し0件」を主張するときは `v2/daily_pipeline.ts` を必ず検索範囲に入れる。
    「参照0件」という機械的な証拠は、**検索範囲が正しいことが前提**だという当たり前を踏み外した。
    追加した `src/lib/backlog.ts` 自体は重複ではない（測る対象が違う）ので残す:
    `reportBacklog` は `collected_data` の**未処理行数**をSQLで数えるもの、`formatBacklog` は
    `buildDailyReport` の**その場の絞り込み上限**（`CANDIDATE_LIMIT` / `TOP_N`）が埋まったかを見るもので、
    後者は行の状態ではないのでSQLでは数えられない。文言をわざと揃えていないのも、
    `[KeyPoints] 未処理の残りn件（1回の上限m件）` は復旧チェックリストが**そのまま探す文字列**だから。

56. **1,800字上限が商品の約束を本当に守れているかを、全121号で確かめた（2026-09-15）**
    — 「1号を3分で読み終わる」は商品の核（cernoval-concept）だが、上限を入れた 2026-09-12 の時点では
    13日分しか測っていなかった。バックアップで公開済み daily 全121号を測り直した。
      - 導入後（09-12〜）3号: 中央 **2分36秒**（1,483字・1,558字・2,928字）
      - 導入前（08-12〜09-11）31号: 中央 **8分52秒**・**31号すべてが3分超**
      - 全期間121号: 119号（98.3%）が3分超
    09-12 は上限を入れた当日でその号だけ4分53秒だが、**翌日以降は一度も超えていない**。
    上限は効いている。
    **過去119号は直さない**: 号は日付の付いた発行物なので、後から中身を削るのは記録の改変になる。
    各号には実測値がそのまま表示されている（`IssueHeader` の「◯分◯秒で読めます」）ので嘘にはならず、
    `/about` の約束も「3分で読み終わる**長さで出します**」＝これから出す号の仕様として書かれている。
    数字は `src/lib/reading-time.ts`（読了時間の唯一の定義場所）に併記した。
    **この過程で測って外れた仮説（再調査しないこと）**:
      - `/about` が実績を誇張しているのでは → 仕様の約束として正しく書き分けられていた
      - 404・error ページがテーマ移行から取り残され、明るい地で読めなくなっているのでは →
        実測すると `text-white` / `text-slate-400` 等のTailwindクラスは globals.css の素の要素規則に
        負けて**効いていない**ため、light/dark どちらでも 6.33:1〜19.42:1 で問題なし
        （クラス自体は死んでいるが見た目に害は無い）
      - サイト全体のコントラスト不足 → 検出器が alpha合成・`oklab()`・`background-clip:text` を
        扱えず**偽陽性ばかり**だった。この粒度では主張できないので撤回（測り直すなら axe 等を使う）

57. **見出しだけが並んで本文が消えていた号が、まだ3号残っていた（2026-09-15）**
    — 2026-09-11 に「旧形の `**ラベル:**` を落としていた」を直し、2026-09-15 に「`### ` で割れない
    箇条書き形式の33号」を救った。それでも**同じ型が2つ残っていた**。公開142号を機械的に測って発見:
    「ハイライトの見出しは出るのに要点が1本も無い号」が **3号**（id=14 / id=19 / id=90）。
    紙面には見出しが5本並ぶだけで、本文が丸ごと消えていた（失敗が部分的でなく**全体的**）。
      - 形1（2026-05-21 / 05-22）: `### 1. 見出し` の下に**箇条書き記号なし**で `**何が起きたか:** 本文`
      - 形2（id=90・2026-06-11の再生成版）: `*   **見出し**` の下に**字下げした素の本文**（記号なし）
    どちらも「箇条書き行だけを要点にする」規則から外れていた。**記号の有無で本文を捨てない**ことにし、
    落とすのは見出し・区切り線・空行だけにした（`isBodyLine`）。
    **効果は全142号で前後比較した**: 要点0の号 3→**0**。増えたのは壊れていた3号だけ（+15 / +15 / +4）、
    **139号は完全に変化なし・減った号ゼロ・見出し数の変化ゼロ**＝外科的。
    紙面の描画率（元の可読字数に対する割合）も 85%未満が **4号→1号** に減った。
    **触らなかったもの**: 残る1号（2026-05-18・81.6%）は `## 🚀 今日のハイライト` と絵文字が
    🔥 でないため 🔥 決め打ちの `extractHighlightSection` から外れているが、**セクションとして
    正しく描かれている**。ハイライト扱いにすると `*   **要約文:** 本文` の1行がまるごと「見出し」に
    なって**かえって悪化する**ので、絵文字の決め打ちは残す。
    **副産物**: 同じ日付の daily が2号ある日を1日見つけた（2026-06-11: id=87 21:01 正常 /
    id=90 23:02 壊れ）。再生成で古い号を消していないため。バックナンバー一覧は日付で並ぶので
    読者にはどちらが出るか不定。**これは未対応**（重複の解消は本人判断・decisions.md 未起票）。

58. **本文が空の号が、バックナンバー一覧からは隠れているのに sitemap・RSS・次号ボタンには出ていた（2026-09-15）**
    — 空の号は実在する: `id=81`（2026-06-09・daily・`content=""`）。2026-06-09 に LLM が空応答を返し、
    それを**検証せずに保存**した事故（[[debug-empty-report-digest]]・生成側は b19d4a4 で修正済み）の
    残骸で、行は消していないので今もDBにある。
    問題は**除外条件が1か所にしか無かった**こと。`getRecentDigests` だけが `length(content) > 0` を
    持っていて、同じくレポートを公開面に出す他の関数には無かった。実データで確認した実害:
      - **前号/次号ナビ**: 2026-06-08 の「次号」と 2026-06-10 の「前号」が、どちらも空の号を指していた
        ＝読者が次号ボタンを押すと**中身ゼロの紙面**に着く導線が2本
      - **sitemap**: `getReportsData(1000, 1)` が種別しか絞っていないので空の号のURLをクローラに申告
      - **RSS**: 同じ関数を使うので、空の号が今日生成されれば `<description>` も本文も空の item が流れる
        （id=81 は6月なので現在の配信範囲には入っていないが、同じ型の穴）
    述語を `EMPTY_REPORT_EXCLUDED` として1か所に置き、4つの関数（`getReportsData` /
    `getRecentDigests` / `getAdjacentReports` / `getLandingDigest`）で共有した。
    **`getLandingDigest` は最初この一覧から漏らしていた**のを、「述語は1か所だけで定義されている」を
    見るテストが生の `length(...) > 0` として捕まえた＝機械検査が実際に仕事をした。
    効果はバックアップで前後比較した: 空の号への導線 **2本→0本**、変化したのは**その2本だけ**
    （06-08 の次号と 06-10 の前号が互いを指すようになった）、他の導線は一切変化なし。
    sitemap/RSS の対象は 143件→142件。
    **入れなかったもの**: `getReportById`（id 直指定）。ここに条件を足すと `/reports/81` が404になる。
    空の号でも当時メールで配ったURLではありうるので、「**新たにリンクしない**」だけに留め、
    既に配ったURLは殺さない（失敗の非対称性）。
    **未対応（本人判断）**: 同じ日付に daily が2号ある日が1日（2026-06-11: id=87 / id=90）。
    `getAdjacentReports` は strict `<` / `>` なので互いに辿れない。行の整理は本番DBの変更になる。

59. **sitemap は公開後の障害で「もうこの13件しかない」と申告する。素直な修正は試して却下した（2026-09-15）**
    — RSS には 0件のとき 503 を返すガードを入れた（㊱）が、**同じ型の sitemap には入っていなかった**。
    `sitemap.ts` の3つの取得は `try { … } catch { /* 入口ページのみ */ }` で囲まれているが、
    3つの取得関数はどれも内部で catch して `[]` を返す fail-open なので、**この catch は一度も走らない**。
    実際に起きるのは「例外」ではなく「空で返る」で、固定13URLの sitemap が **`lastmod = 今` つきで 200**
    で出る＝検索エンジンに新鮮な判子つきで「もうこの13件しかない」と申告する。
    **いまは無害**（`SITE_NOINDEX = true` のあいだ sitemap は DB を1つも引かない・2026-09-14 の止血）。
    発火するのは公開に戻した後だけ。だが公開作業は `SITE_NOINDEX` を false にするだけなので、
    そのとき誰も sitemap を見直さない。
    **修正を実装して測り、却下した**: throw する案を入れて `SITE_NOINDEX=false` + 枠切れDBで
    ローカル本番ビルドを回すと **`next build` が終了コード1で落ちた**。sitemap はビルド時に
    プリレンダされるため、throw はそのままビルドの失敗になる。**障害中こそ修正をデプロイしたいのに
    デプロイ自体ができなくなる**ので、代償が大きすぎる。変更は巻き戻した。
    **残した提案**: `app/sitemap.ts` を `app/sitemap.xml/route.ts`（route handler）に移せば、
    feed.xml と同じ形で `503 + Retry-After + no-store` を返せてビルドも落ちない
    （feed.xml は現に503を返しながらビルドも本番も通っている）。代償は `MetadataRoute.Sitemap` の型と
