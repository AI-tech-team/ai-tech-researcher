...
## 2026-09-12 — 起動スプラッシュを「流れから、一筋を抜く」に作り直し

**決めたこと**: 図形から**点を無くした**（本人の指示「流動的な点のない図形」）。
1本の曲線 `SPLASH_FLOW` を太さ違いで4本重ね、太く淡い3本＝毎日流れてくる記事の量、
細く明るい1本＝そこから抜き出した今朝の筋。厚みの差がそのまま「次元」になる。

- 旧版（7×4の点のうち5点を夜明け色で灯す「選別」）は、意味は正しかったが**粒**に見えて
  流れにならなかった。意味は「量の中から一筋」に置き換えて引き継いでいる。
- **動かすのは transform と opacity だけ**という制約は維持（[[debug-pwa-cold-launch-freeze]]）。
  「線を描く」表現の定番である `stroke-dashoffset` はメインスレッド処理で、
  ハイドレーション中（数秒）は必ず固まるので使わない。代わりに `scaleX(0.04) → 1` で
  「左から流れ込む」を作る＝コンポジタだけで完結する。
  - 副次効果: 途中は波が横に圧縮されるため、明るい一筋が流れの中を縫うように見える。
    層ごとに遅延を変えてあるので、これが「流動」そのものの表現になっている。
- タイミングの不等式は旧版から引き継ぐ: **主役が通り終わる時刻 < フェード開始**。
  いま 0.14s + 0.48s = 0.62s < 0.82s。

**⚠ viewBox は太い線のはみ出しぶんを外に取る**: `0 0 180 92` のままだと、いちばん太い層
（strokeWidth 30）の左右が SVG の箱で**垂直に切れて**、流れが壁にぶつかったように見えた
（スマホ実機のコマ撮りで発覚）。`-18 -10 216 112` に広げて解消。
コマ撮りは `scripts/_shot_splash.mjs`（`document.getAnimations()` を pause して
`currentTime` を固定する）。動きのあるものはスクショ1枚では検証できない。
