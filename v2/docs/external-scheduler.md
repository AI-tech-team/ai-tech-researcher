...
`runDailyReportAndDistribute()` は「その日の号が既にあればスキップ」する（冪等）。
起動経路が外部トリガ・`schedule` の保険・昼の自己修復と3つあるので、重なっても
生成も配信も1日1回に閉じる（重ならないとGemini課金が2回、購読者に同じメールが2通届く）。

### 外部スケジューラが止まったら何が起きるか

| | 止まっても動く | 止まると動かない |
|---|---|---|
| 収集 | ✅ `schedule` の6回が続ける | |
| 朝刊の生成・配信 | ✅ 06:11 JST の保険cronが出す（遅れるが出る） | |
| 知識抽出・埋め込み・翻訳・週次/月次 | | ❌ フル実行の中にあるため走らない |

フル実行には保険cronを置いていない。冪等ガードが無く、遅延込みで外部トリガとほぼ同時刻に
走るため、収集もキーワードGrounding課金も二重になるから。**設定は後回しにしないこと。**
