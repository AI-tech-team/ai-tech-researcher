...
// 必須envの早期検証。未設定だと createClient が不明瞭なエラーで落ち、main()のtry/catchや
// 失敗通知メールにも到達しない（モジュール評価中に死ぬ）ため、ここで分かりやすく落とす。
if (!process.env.TURSO_DATABASE_URL) {
  console.error('[Pipeline] 致命的: TURSO_DATABASE_URL が未設定です。環境変数/シークレット設定を確認してください。');
  process.exit(1);
}
...
async function getRecentTitleCache(): Promise<string[]> {
  ...
  const cutoff = sqlTs(new Date(Date.now() - 14 * 24 * 60 * 60 * 1000));
...
// メールHTMLへ外部由来のDB値(記事タイトル/要約/URL/表示名等)を埋め込む際のエスケープ。
// フィード/LLM由来の値に含まれる <>&"' で属性破壊・HTMLインジェクションが起きるのを防ぐ。
function escapeHtml(s: string): string {
  return String(s ?? '')
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}
// href用: http(s)以外（javascript:/data:等）は'#'に、それ以外は属性エスケープ。
function safeMailHref(u: string | null | undefined): string {
  if (!u || !/^https?:\/\//i.test(u.trim())) return '#';
  return escapeHtml(u);
}
...
async function sendPersonalizedBriefs(reportText: string | null = null) {
  ...
  let reportMd = reportText?.trim() ? reportText : null;
  if (!reportMd) {
    // 空レポートが万一最新dailyでも飛ばし、中身のある最新を使う（ダイジェスト消失の二重防御）。
    const [latest] = await db.select({ content: schema.reports.content })
      .from(schema.reports).where(and(eq(schema.reports.type, 'daily'), sql`length(${schema.reports.content}) > 0`))
...
async function evolveSources() {
  ...
  const thirtyDaysAgo = sqlTs(new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000));
...
${contextText}`,
  ...
  try {
    const thirtyDaysAgoISO = sqlTs(new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000));
  ...
  try {
    ...
    const oneWeekAgoISO = sqlTs(new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000));
    const fourWeeksAgoISO = sqlTs(new Date(now.getTime() - 28 * 24 * 60 * 60 * 1000));
  ...
  try {
    ...
    if (pipelineMode !== 'collect') {
      ...
      try {
        ...
        console.warn('[Pipeline] 週次/月次/evolve/監視 失敗(非クリティカル):', e?.message ?? e);
      }
    }
